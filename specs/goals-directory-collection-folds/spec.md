...
- `specs/directory-title-toolbar/spec.md` 里 Goals 工具栏仍是一排图标（筛/建/归档/回收站）；
- `specs/chrome-plugin-rail/spec.md` 里「Goals 列表语法不改」。
...
- 工具栏一行：左侧横向「新建 Goal」（仍 `data-open-create`，浅色白底灰边），右侧同样风格的筛选图标和原状态筛选面板。外观见 `specs/goals-directory-outline-buttons/spec.md`。
- 当前 Goal 树也是百叶窗段，默认展开；见 `specs/goals-directory-current-fold/spec.md`。
