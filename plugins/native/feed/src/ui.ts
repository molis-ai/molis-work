...
export function renderFeedWorkbench(model: FeedUiModel): string {
  ...
  return `<section class="desktop-work-surface feed-workbench" data-work-surface="feed" data-work-surface-label="Feed" data-feed-workbench data-feed-directory data-feed-preset="feed" data-feed-stage-directory="true" data-loaded="true" data-loaded-preset="feed"${model.active ? "" : " hidden"}>
    ${model.error ? `<div class="feed-error-state" role="alert"><strong>${p.text("Feed 暂时无法载入")}</strong><p>${p.escape(model.error)}</p><button type="button" data-retry-feed-detail>${p.text("重试")}</button></div>` : ""}
    ${renderFeedStageDirectory(model)}
    <div class="feed-detail-empty" data-feed-detail-empty hidden>${p.icon("input")}<h1 data-feed-detail-empty-title>${p.text("正在载入 Item…")}</h1><p data-feed-detail-empty-copy hidden></p><button type="button" data-retry-feed-detail hidden>${p.text("重试")}</button></div>
  </section>`;
}
...
function renderFeedStageDirectory(model: FeedUiModel): string {
  ...
  return `<div class="feed-stage-directory">
    ...
    <div class="feed-stage-tree">
          <div class="feed-stage-list" data-feed-list role="list" aria-label="${p.text("Item 列表")}">${rows}<div class="feed-list-empty" data-feed-empty${entries.length ? " hidden" : ""}><strong data-feed-empty-title>${p.text("这里还没有 Item")}</strong><button type="button" data-feed-clear-filters hidden>${p.text("清除筛选")}</button>${model.demo ? `<button type="button" data-prototype-feed-restore hidden>${p.text("恢复列表")}</button>` : ""}<button type="button" data-feed-add-toggle>${p.text("添加任务")}</button></div></div>
...
function renderPrototypeFeedDetail(entry: FeedUiEntry, selected: boolean, model: FeedUiModel): string {
  ...
  return `<article class="feed-detail feed-detail--prototype" data-feed-detail="${p.escape(entry.entry_id)}" data-feed-detail-item-type="feed" data-prototype-feed-detail${selected ? "" : " hidden"}>
    <header class="feed-detail-header"><div class="feed-detail-kicker"><span>${p.escape(entry.kind_label)}</span><span>${p.escape(entry.source_label)}</span><span>${p.text("仅本页演示")}</span></div><h1>${p.escape(item.title)}</h1><p>${p.escape(item.summary)}</p><div class="feed-detail-meta"><span>${p.icon("link")}${p.escape(prototype.relation)}</span><time datetime="${p.escape(item.source_updated_at)}">${p.formatDate(item.source_updated_at)}</time></div></header>
...
export function renderPersistedFeedItemDetail(model: PersistedFeedDetailModel, selected = true): string {
  ...
  return `<article class="feed-detail" data-feed-detail="${p.escape(model.entry_id)}" data-feed-detail-item-type="feed" data-feed-detail-read="${item.read_at ? "read" : "unread"}"${selected ? "" : " hidden"}>
    <header class="feed-detail-header"><div class="feed-detail-kicker"><span>Feed</span><span>${p.escape(item.source_label || item.source_kind)}</span><span data-feed-read-state>${item.read_at ? p.text("已读") : p.text("未读")}</span><span>${p.escape(dispositionLabel(model.inbox_active ? "inbox" : effectiveDisposition, p))}</span></div><h1>${p.escape(item.title || p.text("未命名消息"))}</h1>${item.summary ? `<p>${p.escape(p.plainText(item.summary))}</p>` : ""}<div class="feed-detail-meta">${item.author ? `<span>${p.icon("user")}${p.escape(item.author)}</span>` : ""}<time datetime="${p.escape(item.source_updated_at)}">${p.formatDate(item.source_updated_at)}</time>${item.source_id ? `<button type="button" data-open-source-record="${p.escape(item.source_id)}">${p.icon("settings")}${p.text("查看来源")}</button>` : ""}${itemUrl ? `<a href="${p.escape(itemUrl)}" target="_blank" rel="noopener noreferrer">${p.text("打开原文")}${p.icon("arrow")}</a>` : ""}</div></header>
    ...
    <footer class="feed-reader-footer"><section class="feed-destination-strip" data-destination-state="${p.escape(model.inbox_active ? "inbox" : effectiveDisposition)}"><span>${p.icon("activity")}${p.text("当前去向")}</span><strong>${p.escape(destination[0])}</strong><small>${p.escape(destination[1])}</small></section><div class="feed-detail-actions" data-feed-actions>${actions}</div><p class="feed-action-status" data-feed-action-status role="status" hidden></p></footer>
  </article>`;
}
...
export function renderSourceDirectory(model: FeedUiModel): string {
  ...
  return `<section class="desktop-directory-panel source-directory" data-directory-panel="sources" data-source-directory hidden><header class="desktop-directory-heading source-directory-heading"><button type="button" data-directory-back aria-label="${p.text("返回上一级")}">${p.icon("back")}</button><span><strong>${p.text("来源")}</strong><small>${p.text("账号、接入源与拉取计划")}</small></span><button class="source-add-trigger" type="button" data-feed-sources-open aria-label="${p.text("添加来源")}">${p.icon("plus")}</button></header><div class="source-directory-tools" data-directory-list-actions><details class="source-filter-menu"><summary aria-label="${p.text("来源筛选")}" title="${p.text("来源筛选")}">${p.icon("filter")}</summary><div class="source-filter-row" role="group" aria-label="${p.text("来源筛选")}"><button class="is-active" type="button" data-source-filter="all">${p.text("全部")}</button><button type="button" data-source-filter="account">${p.text("账号")}</button><button type="button" data-source-filter="public">${p.text("公开 Feed")}</button><button type="button" data-source-filter="attention">${p.text("需处理")}</button></div></details><button class="source-mobile-add" type="button" data-feed-sources-open>${p.icon("plus")}${p.text("添加来源")}</button></div><div class="source-list" data-source-list role="listbox" aria-label="${p.text("来源列表")}">${rows}<div class="feed-list-empty source-list-empty" data-source-empty hidden>${p.icon("search")}<strong>${p.text("没有符合条件的来源")}</strong><button type="button" data-source-filter-reset>${p.text("清除筛选")}</button></div></div><footer class="feed-directory-footer"><span data-source-result-count>${p.text("{count} 个来源", { count: model.sources.length })}</span></footer></section>`;
}
...
export function renderFeedOverlays(model: FeedUiModel): string {
  ...
  const choices = [
    ["custom_rss", "link", "网站与博客", "粘贴 RSS 或 Atom 订阅地址，持续收集新文章。"],
    ["rss", "list", "推荐订阅", "从已有目录中选择一个信息源。"],
    ["web_query", "search", "关键词追踪", "围绕一个主题收集网页搜索结果。"],
    ["youtube_channel", "play", "YouTube 频道", "关注一个频道的新视频。"],
    ["github", "activity", "GitHub 通知", "连接账号，收集与你有关的通知。"],
    ["gmail", "message", "Gmail 邮件", "连接邮箱，按范围收集邮件。"],
  ];
  const sourceChoiceRows = choices.map(([kind, icon, title, description]) => `<button type="button" class="feed-source-choice" data-feed-choose-kind="${kind}"><span>${p.icon(icon)}</span><span><strong>${p.text(title)}</strong><small>${p.text(description)}</small></span>${p.icon("chevron-right")}</button>`).join("");
  const taskPanels = model.sources.map((source) => {
    ...
    return `<section data-feed-task-config="${id}" data-source-detail="${id}" data-prototype="${source.prototype}" hidden>
      ...
      <div class="feed-config-actions">${source.ui_kind === "gmail" || source.ui_kind === "github" ? `<button type="button" data-feed-connect-kind="${source.ui_kind}">${p.text("管理账号连接")}</button>` : ''}</div>
      <details class="feed-task-extra" data-feed-plan-region><summary>${p.text("拉取计划")}</summary><p>${p.text("此处单独保存拉取计划，不会保存上方的任务资料。")}</p><label><span>${p.text("拉取方式")}</span><select data-source-schedule-mode><option value="manual"${source.schedule.mode==='manual'?' selected':''}>${p.text("手动拉取")}</option><option value="interval"${source.schedule.mode==='interval'?' selected':''}>${p.text("定时拉取")}</option></select></label><label${source.schedule.mode==='manual'?' hidden':''}><span>${p.text("间隔（分钟）")}</span><input type="number" min="5" max="10080" value="${interval}" data-source-schedule-interval></label><label class="check-row"${source.schedule.mode==='manual'?' hidden':''}><input type="checkbox" data-source-schedule-enabled${source.schedule.mode==='interval'&&source.schedule.enabled?' checked':''}><span>${p.text("启用定时拉取")}</span></label><div class="feed-plan-actions"><button type="button" data-source-schedule-reset>${p.text("撤销修改")}</button><button type="button" data-source-schedule-save data-source-id="${id}"${source.prototype?' disabled':''}>${p.text("保存拉取计划")}</button></div></details>
      <div class="feed-task-controls"><button type="button" data-feed-source-sync="${id}"${source.prototype||!source.enabled||source.status==='disconnected'?' disabled':''}>${p.text("立即拉取")}</button><button type="button" data-feed-source-toggle="${id}" data-feed-source-enabled="${source.enabled}"${source.prototype?' disabled':''}>${source.enabled?p.text("暂停任务"):p.text("恢复任务")}</button></div>
      ${source.prototype?`<p class="feed-setup-hint">${p.text("这是演示任务。添加一个真实来源后即可保存配置和拉取内容。")}</p>`:''}
      <details class="feed-task-extra"><summary>${p.text("最近拉取")}</summary>${renderSourceRuns(source, p)}</details><details class="feed-task-extra"><summary>${p.text("移除任务")}</summary><p>${p.text("停止拉取，已收集的消息与历史仍会保留。")}</p><button type="button" data-source-delete="retain_history" data-source-id="${id}"${source.prototype?' disabled':''}>${p.text("移除任务并保留历史")}</button></details><p data-source-action-status role="status" hidden></p>
    </section>`;
  }).join("");
  return `<dialog class="feed-source-dialog feed-task-dialog" data-feed-sources-dialog aria-labelledby="feed-source-dialog-title"><div class="feed-task-dialog-shell">
    <header><div><h2 id="feed-source-dialog-title">${p.text("添加拉取任务")}</h2><p data-feed-setup-description>${p.text("先选择你想关注的来源。")}</p></div><button type="button" data-feed-sources-close aria-label="${p.text("关闭")}">${p.icon("x")}</button></header>
    <div class="feed-task-dialog-body">
      <section data-feed-source-choices>${sourceChoiceRows}</section>
      <section data-feed-source-setup hidden><button class="feed-setup-back" type="button" data-feed-setup-back>${p.icon("back")}${p.text("选择其他来源")}</button>
...
function renderSourceDetail(source: FeedUiSource, selected: boolean, model: FeedUiModel): string {
  ...
  const messages = source.messages.length ? `<ul>${source.messages.map((message) => `<li>${p.icon("activity")}<span><strong>${p.escape(message)}</strong><small>${p.escape(source.name)}</small></span></li>`).join("")}</ul>` : `<div class="source-panel-empty">${p.icon("archive")}<strong>${p.text("还没有来源消息")}</strong></div>`;
