...
test("Container pins the Goal canvas, locates directory Goals, and only Frames accept non-Goal work", { timeout: 90_000 }, async (t) => {
  ...
  await waitFor("document.querySelector('[data-goal-momentum]')?.dataset.loaded === 'true' && !document.querySelector('[data-container-tabs]').hidden");
  await waitFor("Boolean(document.querySelector('[data-graph-node][data-goal-id=CORE] [data-graph-frame]'))");
  assert.equal(await evaluate("document.querySelector('[data-container-tab=canvas][aria-current=\"page\"]')?.textContent"), "Goal 画布");
  assert.equal(await evaluate("document.querySelector('[data-container-tab=canvas]').classList.contains('is-pinned')"), true);
  assert.match(await evaluate<string>("document.querySelector('[data-goal-momentum] h1')?.textContent || ''"), /目标关系/);
  ...
  await waitFor("document.querySelector('[data-graph-node][data-goal-id=CORE].is-selected')");
  assert.equal(await evaluate("document.querySelector('[data-goal-node-workspace]').hidden"), true);
  assert.equal(await evaluate("document.querySelector('[data-container-tab=canvas][aria-current=\"page\"]') != null"), true);
  assert.equal(await evaluate("document.querySelectorAll('[data-container-tab]').length"), 1);
  await click('[data-graph-node][data-goal-id="CORE"] [data-graph-open]');
  await waitFor("document.querySelector('[data-goal-node-workspace]').dataset.expandedGoal === 'CORE'");
  assert.equal(await evaluate("document.querySelector('[data-container-tab=canvas][aria-current=\"page\"]') != null"), true);
  ...
  await waitFor("document.querySelector('[data-container-tab=\"CORE\"][aria-current=\"page\"]')");
  ...
  assert.equal(await evaluate("document.querySelector('[data-goal-node-workspace]').hidden"), true);
  await click('[data-container-tab=canvas]');
  await waitFor("document.querySelector('[data-container-tab=canvas][aria-current=\"page\"]') && !document.querySelector('[data-goal-canvas-shell]').hidden");
  await evaluate("document.querySelector('[data-graph-node][data-goal-id=\"CORE\"] [data-graph-frame]').click()");
  await waitFor("document.querySelector('[data-container-tab=\"CORE\"][aria-current=\"page\"]')");
  assert.equal(await evaluate("[...document.querySelectorAll('[data-container-tab]')].map(tab => tab.dataset.containerTab).join(',')"), "canvas,CORE");
  ...
  await click('[data-plugin-strip] [data-plugin-id="goals"]');
  await waitFor("document.body.dataset.desktopSurface === 'goal' && document.querySelector('[data-container-tab=\"CORE\"][aria-current=\"page\"]')");
  ...
  assert.match(await evaluate<string>("document.querySelector('[data-frame-block].is-expanded').textContent || ''"), /终端仍在 Goal 工作框/);
  ...
  await waitFor("document.querySelector('[data-container-tab=canvas][aria-current=\"page\"]') && document.querySelector('[data-graph-node][data-goal-id=CORE].is-selected')");
  assert.equal(await evaluate("document.querySelector('[data-goal-node-workspace]').hidden"), true);
  assert.equal(await evaluate("document.querySelector('[data-container-tab=\"CORE\"]') != null"), true);
  ...
  await click('[data-plugin-strip] [data-plugin-id="goals"]');
  await waitFor("document.body.dataset.desktopSurface === 'goal' && document.querySelector('[data-container-tab=\"CORE\"]')");
  await click('[data-container-tab-close="CORE"]');
  await waitFor("document.querySelector('[data-container-tab=canvas][aria-current=\"page\"]') && !document.querySelector('[data-container-tab=\"CORE\"]')");
  await evaluate("document.querySelector('[data-graph-node][data-goal-id=\"CORE\"] [data-graph-frame]').click()");
  ...
  await waitFor("document.body.dataset.desktopSurface === 'home'");
  await click('[data-plugin-strip] [data-plugin-id="goals"]');
  await waitFor("!document.querySelector('[data-container-tabs]').hidden");
  assert.equal(await evaluate("document.querySelector('[data-container-tab=\"CORE\"]')"), null);
  await navigate(() => command("Page.navigate", { url: origin + "/projects/" + projectId + "/" }, sessionId));
  await waitFor("document.body.dataset.desktopSurface === 'home'");
  await click('[data-plugin-strip] [data-plugin-id="goals"]');
  await waitFor("document.querySelector('[data-container-tab=\"CORE\"]')");
  await click('[data-container-tab="CORE"]');
  await waitFor("document.querySelector('[data-frame-block]')");
