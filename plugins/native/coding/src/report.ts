...
export function createCodingExecutionReport(input: {
  ...
  return {
    ...
    body_markdown: [
      ...
      ...(frozen.subagent_workspaces?.length ? [`## 本轮独立目录\n${literal(frozen.subagent_workspaces.map(item => `${item.workspace_id}：${item.directory.canonical_path}`).join("\n"))}\n目录按本轮授权固定；子任务结果须在原子任务记录中核对，不代表已经整合到主工作区。`] : []),
