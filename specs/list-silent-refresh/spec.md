...
允许：`specs/list-silent-refresh/`；`apps/workbench/src/scripts/client/{navigation-feed,navigation-inbox,events-primary,events-secondary}.ts`；`apps/workbench/src/plugin-workbench.ts`；`apps/workbench/src/renderer.ts`；`apps/local-host/src/{inbox,schedule}-native-plugin-http.ts`；`apps/local-host/src/web-request.ts`；`plugins/native/{inbox,schedule,pages,form,dataset,ppt,functions,lingguang}`；对应测试。
