...
test("Container pins the Goal canvas, locates directory Goals, and only Frames accept non-Goal work", { timeout: 90_000 }, async (t) => {
  ...
  await waitFor("Boolean(document.querySelector('[data-tab-workspace] .tab-item[data-tab-kind=home]'))");
  ...
  await evaluate("document.querySelector('[data-container-tab=canvas]').click()");
  ...
  await evaluate("document.querySelector('[data-graph-node][data-goal-id=\"CORE\"] [data-graph-frame]').click()");
  await waitFor("document.querySelector('[data-container-tab=\"CORE\"][aria-current=\"page\"]')");
  assert.equal(await evaluate("[...document.querySelectorAll('[data-container-tab]')].map(tab => tab.dataset.containerTab).join(',')"), "canvas,kanban,CORE");
  ...
  await evaluate("document.querySelector('[data-container-tab=\"CORE\"]').click()");
  await waitFor("document.querySelector('[data-container-tab=\"CORE\"][aria-current=\"page\"]') && !document.querySelector('[data-goal-frame-surface]').hidden");
  ...
  await evaluate("document.querySelector('[data-container-tab-close=\"CORE\"]').click()");
  await waitFor("document.querySelector('[data-board-view-tab=canvas][aria-current=\"page\"]') && !document.querySelector('[data-container-tab=\"CORE\"]')");
  await evaluate("document.querySelector('[data-graph-node][data-goal-id=\"CORE\"] [data-graph-frame]').click()");
  ...
  await navigate(() => command("Page.navigate", { url: origin + "/projects/" + projectId + "/" }, sessionId));
  await waitFor("Boolean(document.querySelector('[data-tab-workspace] .tab-item'))");
  await click('[data-plugin-strip] [data-plugin-id="goals"]');
  await waitFor("document.body.dataset.desktopSurface === 'goal' && document.querySelector('[data-container-tab=\"CORE\"]')");
  await evaluate("document.querySelector('[data-container-tab=\"CORE\"]').click()");
  await waitFor("document.querySelector('[data-frame-block]')");
