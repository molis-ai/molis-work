...
export const JELLY_CALENDAR_CLIENT_SCRIPT = String.raw`
  ...
  const agendaHtml=(items,selectable=false)=>{
    ...
    return '<div class="jelly-agenda">'+dates.map((date)=>'<section class="jelly-agenda-group"><h2>'+esc(dateLabel(date))+'</h2>'+items.filter((item)=>item.start_date===date).map((item)=>'<div class="jelly-agenda-row">'+(selectable&&!item.series_id&&!item.completed_at?'<input type="checkbox" class="mw-checkbox" data-jelly-select-item="'+esc(item.id)+'" aria-label="'+tx('选择事项')+' '+esc(item.title)+'" '+(calendarSelected.has(item.id)?'checked':'')+'>':'')+btn(item.completed_at?'恢复事项':'完成事项','data-jelly-toggle-item="'+esc(occurrenceKey(item))+'"',item.completed_at?'completed':'circle')+occurrenceHtml(item)+(!item.series_id&&item.start_date===item.end_date&&item.start_time===null?btn('向上移动','data-jelly-reorder-item="'+esc(item.id)+'"','chevron-up'):'')+'</div>').join('')+'</section>').join('')+'</div>';
  };
  ...
  const handleCalendarClick=(target,event)=>{
    ...
    if(target.hasAttribute('data-jelly-item-copy'))return void run(async()=>{if(!editingItem)return;const copy={...editingItem,id:undefined,completed_at:null,completion_description:''};await command({type:'item.create',item:copy});$('[data-jelly-item-dialog]').close();showNote(L('已复制为独立事项'));});
