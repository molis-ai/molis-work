...
允许：`specs/archive/lingguang-plugin/`；`packages/contracts/src/modules/lingguang.ts`；`plugins/native/lingguang/`；Workbench catalog/壳接线；Local Host HTTP；`scripts/workspace-packages.mjs`；SSOT 一行；`tests/lingguang-plugin.test.ts` 及被个人插件名单断言带出的测试修正。
