...
本文件是这次目录主操作外观的唯一需求书。结构仍走 `specs/archive/goals-directory-collection-folds/spec.md`（横条 + 右筛选）；控件手感走 `specs/archive/coss-control-language/spec.md` 的主按钮合同。
...
做：Goals 目录 `data-open-create` 这条用 Coss 主按钮（class `button-primary` + 共享 `--action` 层）。加号跟文字同色。筛选漏斗外观见 `specs/archive/goals-filter-primary-button/spec.md`。
...
2. 筛选仍在右侧；外观合同见 `specs/archive/goals-filter-primary-button/spec.md`。
3. 点击仍打开创建面板。
