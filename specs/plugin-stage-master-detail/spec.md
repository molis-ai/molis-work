...
本文件是这次体验变更的唯一需求书。它覆盖 Feed 左右重复分类、Inbox / Artifacts / Shelf 另占第二栏，以及 Artifacts 仍用目录两行组件、进页自动展开上次版本。Sessions 已按 `specs/session-stage-list/spec.md` 落地。
...
- Shelf：三组 fold（材料 / 生成结果 / 剪贴板）进舞台列表，空组也显示。失败条目 → 三角，否则勾。点材料、结果或剪贴板一行才展开预览。展开后藏搜索 / 选文件 chrome，避免挤进 `--tree-width`；返回列表后再选文件。DropAgent 预览、对照、TTY、jobs、轮盘、行 glyph、`--da-*` 留在行和预览上。列表背景走 plugin-stage paper。
...
4. 点 Shelf：材料 / 生成结果 / 剪贴板三个 fold 铺满；点一份材料才出 DropAgent 预览和底栏。空组仍在。
...
- Shelf 的 `data-shelf="directory"` 留在舞台 list 上，避免重写整个 client；`selectItem` / `selectClip` 后 `data-expanded="true"`。
...
2. 未点行时没有可见详情；Feed 空任务仍有 fold；分组 mark 是 `check` 或 `alert`。
...
7. Shelf 三组 fold 在舞台列表（空组也在）；点行才展开预览；提取 PDF / 对照 / 底栏仍可用。完成等级 3。不改用户真实库。
