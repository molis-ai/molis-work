...
const host = new MolisWorkLocalHost({ homeDirectory: home, completeText: async () => "MCP 题目" });
const ref = molisWorkHostProjectReference({ databasePath: join(home, `${projectId}.sqlite`), boardId: projectId, projectId });
await host.withProject(ref, runtime => runtime.coordinator.initializeBoard({ board_id: projectId, title: "MCP Form fixture", actor_id: `fixture-${projectId}`, idempotency_key: "form-mcp-fixture-init" }));
const ports = createActionMcpPorts({ service: host.actionClient(ref), serverInfo: { name: "form-actions", version: "1" },
  context: () => ({ actor_id: `fixture-${projectId}`, project_id: projectId, audience: "mcp", permissions: access === "read" ? ["form:read"] : FORM_ACTION_PERMISSIONS }),
});
