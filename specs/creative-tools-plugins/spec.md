...
完成等级：**4 内部完整**。不宣称可发布。不发 Artifact。对外 MCP 按 `specs/plugin-outbound-mcp/spec.md` 登记，默认关。
...
允许：`specs/creative-tools-plugins/`；`packages/contracts` 三个 module；`plugins/native/{form,dataset,ppt}`；Workbench catalog/壳接线；Local Host HTTP；`scripts/workspace-packages.mjs`；SSOT 三行；对应测试。

禁止：改 Goals/Artifacts 事实；把开关写进 Functions 设置；新建 MCP 包。对外 MCP 的目录合成、闸门、Host 适配表以 `specs/plugin-outbound-mcp/spec.md` 为准。

## 验收标准

1. 三个插件出现在项目侧栏，不经项目「添加插件」。
2. Forms：创建→改题→上下移动→预览提交（多选为真实选项）→结果计数；重启进程记录仍在。
3. Dataset：行列编辑、CSV 导入前确认覆盖、导出、版本回滚；重启仍在。
4. PPT：多页编辑、预览随改随看、JSON 导出；重启仍在。
