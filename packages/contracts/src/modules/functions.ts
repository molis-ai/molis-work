...
export const FUNCTION_AUTHORING_SUBJECTS: readonly FunctionAuthoringSubject[] = [
  ...
  { subject_kind: "source", title: "来源故障" },
  { subject_kind: "home_event", title: "首页事件" },
  { subject_kind: "session", title: "Session" },
  { subject_kind: "mcp_invoke", title: "Agent 调用" },
];
  ...
  return [
    {
      ...
      title: "首页事件",
      when: "首页出现这条事件、点开详情时",
      configure_at: "首页详情里的「卡底判断」",
      effect: "只挑亮一两个按钮。写操作不会自动执行。",
    ...
    },
    {
      ...
      title: "Inbox 落地",
      when: "Inbox 条目被创建时",
      configure_at: "Inbox 列表的「下一步判断」",
      effect: "只改「做完了 / 忽略」谁出现，不自动改状态。",
    ...
    },
    {
      ...
      kind: "event",
      title: "Feed 捕捉",
      when: "捕捉规则命中一条 Feed 消息时",
      configure_at: "Feed 任务的捕捉规则",
      effect: "只改「加入 Inbox」是否出现，不自动写入。",
    ...
    },
    {
      ...
      kind: "mcp",
      title: "给 Agent 调用",
      when: "Agent 调用 molis_work_v1_functions_invoke 时",
      configure_at: "发布后即可，不用现场开关",
      effect: "返回挑中的动作。MCP 写工具不会因为判断结果自动执行。",
