...
- `specs/archive/project-chrome-span-rail/spec.md` 里项目条在 titlebar **下面**单独一行；
- `specs/archive/chrome-plugin-rail/spec.md` 里「项目切换不进 titlebar」和「插件栏从项目条下一行起」。
...
- 有目录时：项目条对齐目录列（默认 240px，拖过跟 `--tree-width`）。网页从插件栏右边开始，不把插件栏算进宽度；右缘贴目录右缘；搜索/设置/收起仍贴这条右缘。
- Goals 不占第二栏，但项目条仍按同一目录列铺满，不走首页收窄。首页/插件市场/用户收起目录仍按内容收窄。
- 桌面红绿灯仍占 titlebar 左 88px；项目名从让位之后开始，不压灯。
- 上一步/下一步跟标签走，从目录右缘起。
- 下面一行：插件栏、目录、主内容同顶同底。
- 首页/收起目录：项目条在 titlebar 里按内容收窄，不盖住标签。Goals 见 `specs/archive/goal-titlebar-directory-align/spec.md`。
...
1. 打开 Goals：一行里左边项目条仍占目录列宽（从插件栏右边起），右边是上一步/下一步和标签；下面只有插件栏和主内容顶边齐。
2. 回首页：项目名仍在 titlebar 左侧并按内容收窄，标签还在；插件栏和首页内容同顶。
3. 桌面：红绿灯可点，项目名在它右边。
