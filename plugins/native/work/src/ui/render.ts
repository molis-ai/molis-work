...
export function renderWorkSessionSurface(surface: WorkUiSurface, model: WorkUiModel): string {
  ...
  function renderSessionContent(item: ProjectSessionRecord): string {
    if (item.contentMode === "native" || item.contentMode === "fallback") {
      ...
      return `<div class="session-content-state" data-session-content-list>${icon("activity")}<div><h3>${native ? L("按需读取最近执行") : L("读取 GoalBoard 执行记录")}</h3><p>${native ? L("GoalBoard 会分页读取原 Runtime 的最近摘要，并合并本地 TUI 记录；超大历史不会整条载入。") : L("只读取 GoalBoard 已持久化并能证明的 TUI 与执行事实。")}</p><button class="document-action" type="button" data-session-content-load>${L("读取内容")}</button></div></div>`;
    }
    return `<div class="session-content-state" data-session-content-list>${icon("lock")}<div><h3>${L("这个 Runtime 不能读取 Session 内容")}</h3><p>${L("GoalBoard 只显示已确认的 Session ID、项目、Goal 和工作目录，不猜测执行过程。")}</p></div></div>`;
  }
