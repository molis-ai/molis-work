...
插入：`/`、`+`、手柄拖拽排序（跟手预览 + 落点线）、插入线。不要 Markdown 块、顶栏插入、缩放、行引擎、三栏、连线。菜单里仍保留上移/下移。
...
- A9 多栏并排、A14 分享链接。A11 文件导入已由 2026-09-22 用户请求加入范围，见下文。
...
用户可在 Pages 列表点击「导入」，上传 Notion 的 Markdown & CSV / HTML 导出 ZIP、飞书等工具导出的 DOCX / Markdown 或通用 HTML / TXT / CSV。顺序为：选择文件 → 本机解析并预览转换提示 → 勾选文档与目标文件夹 → 原子创建 Pages → 打开编辑并刷新读回。预览阶段不创建文档，损坏文件导致整批失败；同一次提交重试不重复创建，也不覆盖导入后的编辑。

沿用既有 ProseMirror 正文、项目分区、文件夹和 PagesStore，增加格式转换及导入收据，不引入来源同步服务。标题、常用格式、嵌套列表、待办、表格和代码转为可编辑块；图片/附件、复杂表格和相对链接的降级必须在预览可见。ZIP 子页面平铺导入，不重建来源树。账号连接、私有链接直导、持续同步和原平台评论/权限/历史版本仍不在文件导入实现内，不能宣称已支持。

验证覆盖：中文、多文件与 ZIP、DOCX、预览零写入、选择导入、保存/刷新、坏文件恢复、批量回滚、重试幂等、项目与文件夹隔离、压缩包边界、危险 HTML/链接过滤。测试文件为 `tests/pages-import*.test.ts`；真人对实际来源导出包的验收单独记录。
