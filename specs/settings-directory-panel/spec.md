...
项目设置从 chrome 齿轮进入同一套壳，见 `specs/project-settings-directory-panel/spec.md`。独立 `/settings/*` 与 `/projects/{id}/settings*` 直达 URL 仍保留给项目列表、书签和既有滚动合同。
...
- 不做账号登录、头像上传、多账号。
- 项目设置进壳见 `specs/project-settings-directory-panel/spec.md`。
- 不把设置做成可卸载插件或市场项。
- 不删独立 `/settings/*` 页。
...
- 项目设置从 chrome 齿轮进同一套壳，见 `specs/project-settings-directory-panel/spec.md`。
- 账号管理后续另开任务。
