...
Host 把 Attention 条目和关联对象的展示信息交给 UI contribution；HTTP 路由表拥有 `/api/inbox` 匹配，Host 注入 list / setStatus，以及可选的 board 级 `inbox.next` 判断绑定（HTTP 仍在，开关在 Functions 编辑器）。插件不读 Feed 或 Goals 表，也不复制原文。不 import Functions 插件实现。
...
| [src/route-handlers.ts](src/route-handlers.ts) | 列表、状态与 board 级绑函数 HTTP |
