...
test("project policy document selects the last active project binding, preserves explanations and uses default values when absent", () => {
  ...
  assert.match(html, /<section class="settings-section"><h2>高级执行与检查规则<\/h2>/);
  ...
  assert.match(html, /<select id="project-goal-mode" name="goal_mode" class="mw-select">/);
  assert.match(html, /class="mw-switch"/);
  assert.doesNotMatch(html, /settings-switch/);
