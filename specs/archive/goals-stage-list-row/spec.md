...
- `apps/workbench/src/styles/goal-canvas.ts`：舞台列表用 grid 列；`.tree-leading` 按 `(depth + 1) * 16px` 只缩进标题簇，让根 Goal 相对分类标题再缩一档（见 `specs/archive/goal-list-collection-indent/spec.md`）。
