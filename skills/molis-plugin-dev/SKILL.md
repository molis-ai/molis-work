...
4. **还要给谁用？** 只给人点 → UI + HTTP + 客户端。也要给 Agent → 再加 MCP。要让判断亮按钮 → 先有现场按钮，再 `behaviors` / `function_scenes`。要通知别的插件 → 事件。要拉外部世界 → integration。
...
| `app` | Plugin Runtime **真的** `start()`，兑现 views/routes/mcp/behaviors | Coding、Files、Git、Workspace、Diff、Text stats |
...
7. 现场 `behaviors`：只登记画面上已能点的下一步处置。四问见手册「事件去向的动作名单」。
8. 接到 Functions：函数本身在 Functions 插件里写，不是本插件 Manifest。本插件只登记场景、对象种类、现场按钮，再按 [host.md](host.md)「接到 Functions」接线。新去向是平台改动。判断不自动写、不长新按钮。
9. `mcp_exports`：Agent 与人走同一读写。本地 `tool_id`，Host 盖 `molis_work_v1_<短名>_<tool_id>`，并自动进行为总表。schema 禁止身份字段，含 `runtime_actor_id`。新贡献默认关。生产 `tools/call` 未接 Runtime 的 app 不要填。不要再为同一个工具写一条 `behaviors`。
...
| 判断亮哪颗按钮 | 现场按钮 → behaviors → function_scenes | 先写函数再发明按钮 |
| Agent 做同一件事 | `mcp_exports` | 新 MCP 进程 / 写死 tool-catalog |
...
- 只改 Manifest `function_scenes` 就当 Functions「用在哪」多了一行。
