...
- 目录按 Codex / Claude / Grok 常见账号连接铺开；每张卡用该服务真实 App 图标（见 `specs/archive/connectors-catalog-complete/spec.md`）。能连的只有 GitHub 和 Gmail。
...
7. 占位服务已改为同一套 live 目录；见 `specs/archive/connectors-live-catalog/spec.md`。
