...
2. 首页列表区没有「点上面的插件」类说明。目的地与列表之间的横线已由 `specs/archive/directory-list-divider/spec.md` 去掉。**通过**（隔离 4188：浅色桌面、深色首页、390 抽屉）。
3. Goal 行高仍为 36px，标题不换行，状态不被挤掉。**通过**（e2e + 浅色 Goals 截图）。Sessions 随后由 `specs/archive/session-directory-goal-row/spec.md` 对齐为同一单行语法。
