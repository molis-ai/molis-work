...
**标签打开与分组：** 单击添加、不替代以 `specs/additive-tabs/spec.md` 为准；用户手动分组以 `specs/chrome-tabs-preview-groups/spec.md` 为准。本文件仍管分栏、首页默认、市场/设置独占和母标签作为插件主表面的身份。
...
1. 打开项目只见首页标签，不是画布。
2. 点插件打开该插件主表面：Goals 母标签文案是「画布」不是第二个 Goals。不一样的才新增、一样的只激活、分屏允许重复，见 `specs/additive-tabs/spec.md`。单栏时工作区标签在 titlebar；分栏后每栏自己一条。画布/看板在母标签页右上角切换。标题栏不叠 Goal 画布/看板。
