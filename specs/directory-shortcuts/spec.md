...
状态：已实现，本机 4173 只读验收通过。完成等级 **3：功能可用**。不宣称可发布。不改用户真实库内容做演示、不提交、不发布。

本文件是本次体验变更的唯一需求书。它覆盖 `specs/poetic-project-home/spec.md` 与 `specs/home-directory-visual-unify/spec.md` 里「快捷方式在首页构图里」的位置约定，不改存储、校验、外链和编辑弹窗语义。
