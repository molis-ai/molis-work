...
状态：标题与操作同一行已被 `specs/archive/chrome-plugin-rail/spec.md` 取代（操作改到标题下方）。本文件仍保留「目录内不再放搜索框、各插件共用操作标记」的合同。完成等级 **3：功能可用**。不宣称可发布。不改用户真实库、不提交、不发布。
...
- Goals：目录内不再有搜索框。「折叠全部」已由 `specs/archive/goals-directory-remove-collapse-all/spec.md` 删除。新建横条、筛选位置、归档/回收站百叶窗见 `specs/archive/goals-directory-collection-folds/spec.md`。
...
3. Goals 状态筛选、新建、归档、回收站可用；Sessions 筛选 / Feed 筛选 / Inbox 分段 / 来源添加可用。**通过**（Goals / Sessions / Feed / 来源筛选面板宽度贴近目录，不挤在图标里；Inbox「历史」切到空态文案）。「折叠全部」后来删除，见 `specs/archive/goals-directory-remove-collapse-all/spec.md`。
