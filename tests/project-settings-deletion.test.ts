...
test("project settings deletion requires authorization and confirmation, closes the open project, preserves workspace files, and replays its receipt", async t => {
  ...
  assert.match(await initial.text(), /data-project-delete-dialog/);
  const englishPage = await (await fetch(origin + prefix + "/settings/general", { headers: { "accept-language": "en" } })).text();
  assert.match(englishPage, /<h1 id="settings-title">General<\/h1>/);
