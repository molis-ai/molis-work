...
状态：正式产品接缝仍为拟议设计；`ui/` 已按用户批准的 [视觉基准图](ui/design/approved-comp.png) 实现 V3 浏览器本地高保真演示，不代表下述 Agent / Host / 插件运行时集成或最终用户验收已完成。产品范围、阶段与验收以 [spec.md](spec.md) 为准。
...
演示复用现有素材预览控制器和本地状态，没有新增调度服务或通用画布框架。模型解析、Jev 选择与 Agent 调度为 Mock，保存、筛选和 CSV 导出由实际本地代码执行，尚未接通正式运行时；V3 使用独立的草稿、使用版本和素材存储键。当前浏览器实操证据、存储键与待验证项统一记录在 [演示说明](ui/README.md)。
...
现有 `coding-surface.ts` 中的装配不能直接充当生成插件的持久运行实现：它面向已知定义，且该调用点的 `privateStorageFor` 是空实现。生成插件必须使用真正的按安装隔离存储；不复制这个占位行为。
