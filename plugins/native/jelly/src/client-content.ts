...
export const JELLY_CONTENT_CLIENT_SCRIPT = String.raw`
  ...
  const blockHtml=(block,index)=>{
    ...
    let lead=block.kind==='task'?'<input type="checkbox" class="mw-checkbox" data-jelly-task-check="'+esc(block.id)+'" aria-label="'+tx('完成任务块')+'" '+(block.completed_at?'checked':'')+'>':block.kind==='bullet'?'<span class="jelly-block-mark">•</span>':block.kind==='numbered'?'<span class="jelly-block-mark">'+(index+1)+'.</span>':'';
  ...
  const decompose=async()=>{
    ...
    openGeneric('拆成任务','<p class="jelly-muted">'+tx('先生成可编辑的任务计划，确认后才会写入日历。')+'</p>'+(captured?'<label class="jelly-check"><input type="checkbox" class="mw-checkbox" data-jelly-plan-use-selection checked>'+tx('只拆解当前选中文字')+'</label><pre class="jelly-import-preview">'+esc(captured.text)+'</pre>':'')+'<label class="jelly-field">'+tx('补充要求')+'<textarea class="mw-textarea" rows="3" data-jelly-plan-instructions placeholder="'+tx('例如：按优先级拆分，每项控制在半小时内。')+'"></textarea></label><label class="jelly-check" style="margin-top:12px"><input type="checkbox" class="mw-checkbox" data-jelly-plan-manual>'+tx('按原文逐行拆分，不调用模型')+'</label>','生成预览',async()=>{
