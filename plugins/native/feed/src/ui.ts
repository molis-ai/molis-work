...
export function renderFeedDirectory(model: FeedUiModel): string {
  return `<section class="desktop-directory-panel feed-directory" data-directory-panel="feed" data-feed-preset="${model.preset}" hidden></section>`;
}
...
function renderFeedStageDirectory(model: FeedUiModel): string {
  ...
  const tasks = model.sources.map((source) => {
    const count = entries.filter((entry) => entry.item?.source_id === source.source_id).length;
    const fact = [source.status_label, source.last_fetch_label, p.text("{count} 个 Item", { count })]
      .filter(Boolean)
      .join(" · ");
    return `<article class="feed-stage-task" data-feed-task="${p.escape(source.source_id)}">
      <button class="feed-stage-task-head" type="button" data-feed-task-toggle="${p.escape(source.source_id)}" data-source-entry-id="${p.escape(source.source_id)}" data-source-kind="${p.escape(source.ui_kind)}" data-source-status="${p.escape(source.status_kind)}" data-source-search-value="${p.escape(`${source.name} ${source.type_label} ${source.account_label || ""}`.toLocaleLowerCase())}" aria-expanded="false">
        <span class="feed-stage-chevron" aria-hidden="true">${p.icon("chevron-right")}</span>
        <span class="feed-stage-task-copy"><strong>${p.escape(source.name)}</strong><small>${p.escape(fact)}</small></span>
      </button>
      <div class="feed-stage-task-body" data-feed-task-body hidden></div>
    </article>`;
  }).join("");
  ...
  return `<div class="feed-stage-directory">
    ...
    <div class="feed-stage-tree">
      <article class="feed-stage-task is-open" data-feed-task="all">
        <button class="feed-stage-task-head" type="button" data-feed-task-toggle="all" aria-expanded="true">
          <span class="feed-stage-chevron" aria-hidden="true">${p.icon("chevron-right")}</span>
          <span class="feed-stage-task-copy"><strong>${p.text("全部")}</strong><small>${p.text("所有来源的流水")} · ${p.text("{count} 个 Item", { count: entries.length })}</small></span>
        </button>
        <div class="feed-stage-task-body" data-feed-task-body>
          <div class="feed-stage-list" data-feed-list role="listbox" aria-label="${p.text("Item 列表")}">${rows}<div class="feed-list-empty" data-feed-empty${entries.length ? " hidden" : ""}><strong data-feed-empty-title>${p.text("这里还没有 Item")}</strong><button type="button" data-feed-clear-filters hidden>${p.text("清除筛选")}</button>${model.demo ? `<button type="button" data-prototype-feed-restore hidden>${p.text("恢复列表")}</button>` : ""}<button type="button" data-feed-add-toggle>${p.text("添加任务")}</button></div></div>
        </div>
      </article>
      ${tasks}
      <div class="feed-stage-add" data-feed-add>
        <button class="feed-stage-add-toggle" type="button" data-feed-add-toggle>${p.icon("plus")}${p.text("添加任务")}</button>
        <div class="feed-stage-add-form" data-feed-add-form hidden>
          <label><span>${p.text("名称")}</span><input data-feed-add-name maxlength="80" autocomplete="off" placeholder="${p.text("例如：产品观察")}"></label>
          <label><span>${p.text("来源地址")}</span><input data-feed-source-value="custom_rss" inputmode="url" autocomplete="off" placeholder="https://example.com/feed.xml"></label>
          <div class="feed-stage-add-actions">
            <button class="button-primary" type="button" data-feed-source-register="custom_rss">${p.text("开始拉取")}</button>
            <button type="button" data-feed-sources-open>${p.text("其它来源")}</button>
          </div>
          <p class="form-error" data-feed-add-error role="alert" hidden></p>
        </div>
      </div>
    </div>
    ${model.demo ? `<button class="feed-stage-demo" type="button" data-prototype-feed-empty-state>${p.text("预览空状态")}</button>` : ""}
  </div>`;
}
...
function renderFeedStageItem(entry: FeedUiEntry, model: FeedUiModel): string {
  const p = model.primitives;
  const sourceId = entry.item?.source_id || "";
  const revision = entry.item?.revision;
  const ignore = entry.prototype
    ? `<button class="feed-stage-ignore" type="button" data-prototype-feed-action="ignore">${p.text("忽略")}</button>`
    : entry.item_id && revision != null
      ? `<button class="feed-stage-ignore" type="button" data-feed-action="archive" data-feed-item-id="${p.escape(entry.item_id)}" data-feed-revision="${revision}">${p.text("忽略")}</button>`
      : "";
