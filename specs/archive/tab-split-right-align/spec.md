...
本文件纠正 `specs/archive/chrome-plugin-rail/spec.md` 验收 9 在桌面 `?desktop=1` 下的实现：分屏钮要贴窗口 titlebar 右缘，不是贴在被拖动热区挤窄之后的标签条右缘。
...
1. 只有首页一张标签：加号和分屏贴在 titlebar 最右侧，标签和加号之间是空。
...
允许改：`apps/workbench/src/styles/immersive-navigation.ts`、`apps/workbench/src/styles/tab-workspace.ts`、`apps/workbench/src/scripts/client/tab-workspace.ts`；`specs/archive/chrome-plugin-rail/spec.md` 验收 9 一句；`tests/immersive-directory.e2e.test.ts`。
