...
export function renderMolisWorkWorkbenchStylesheet(): string {
  return `${STYLES}${MORE_STYLES}${RESPONSIVE_STYLES}${VISUAL_FOUNDATION_STYLES}${TRASH_GOAL_STYLES}${PROJECT_OPERATIONS_STYLES}${ARTIFACT_EMBED_STYLES}${ARTIFACT_WORKBENCH_STYLES}${IMMERSIVE_NAVIGATION_STYLES}${IMMERSIVE_DIRECTORY_STYLES}${PROJECT_HOME_STYLES}${GOAL_CANVAS_STYLES}${PLUGIN_STAGE_STYLES}${TAB_WORKSPACE_STYLES}${DETAIL_READING_STYLES}${PROJECT_GUIDANCE_SETTINGS_STYLES}${PROJECT_RULES_SETTINGS_STYLES}${PLANNING_SETTINGS_STYLES}${SETTINGS_STYLES}${PROJECT_SETTINGS_PAGE_STYLES}.document-pane.is-syncing .goal-document { animation: none; }${PAGE_CHROME_OVERLAY}${LINEAR_DENSITY_STYLES}${PAGE_PRIMITIVE_TAIL}${SHELF_STYLES}${FUNCTIONS_STYLES}${FORM_STYLES}${DATASET_STYLES}${PPT_STYLES}${LINGGUANG_STYLES}${SCHEDULE_STYLES}${TYPEFACE_STYLES}`;
}
