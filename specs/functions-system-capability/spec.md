...
5. 场景绑定在现场，不另做总控台：Feed 任务捕捉规则用已发布函数下拉（`feed.capture`，按规则 ref）；Inbox 列表有 `inbox.next` 选择器；首页详情有 `home.dock` 选择器。各场景下拉只列出选项 key 全部落在该场景行为池里的已发布 Choice。绑错场景返回 400。落地时（Inbox 创建 / 捕捉命中）再判断，不在点开第三栏时打模型。Feed 详情落地建议只改「加入 Inbox」是否出现；打开原文、保存为资料、升格 Goal、忽略不被藏。判断不自动加入 Inbox。
...
- Inbox / 首页绑函数走各自 HTTP：`GET|POST /api/inbox/judgment`、`GET|POST /api/home/dock-judgment`。body 为 `{ function_key }`，`null` 或空字符串解开。各场景下拉由 Host 对已发布函数做 `functionFitsScene` 过滤后注入。`MolisWorkWebView.function_scenes` 带 `inbox_next_functions` / `home_dock_functions` / `feed_capture_functions`。`inbox.next` 与 `home.dock` 都是 board 级绑定（无 ref）。
...
- `home.dock` 场景池含接着做 / 做完 / 忽略 / 重新授权 / 问问 / 打开。判断落地时的 `offered_behavior_ids` 按场景收窄：Inbox 下一步只有做完/忽略，Feed 捕捉只有进 Inbox/打开，首页卡底是总表里对该对象已兑现路径的处置。`MolisWorkWebView.function_scenes.dock_behaviors` 把这些处置的 id、标题、对象类型交给首页。
...
14. 落地建议只含 `feed.open` 时，详情不显示「加入 Inbox」，「打开原文 / 保存为资料 / 升格 Goal / 忽略」仍在。空或非法建议时「加入 Inbox」仍在。点击才写入，判断本身不自动加入 Inbox。
15. Inbox 下一步下拉含内置 `system_pick_inbox_next`，不含 `system_admit_inbox`；把「是否进 Inbox」绑到 `inbox.next` 返回 400。
16. Feed 捕捉下拉含 `system_admit_inbox`，不含 `system_pick_home_dock`；把首页卡底函数绑到捕捉规则返回 400。首页卡底下拉含 `system_pick_home_dock`，不含 `system_admit_inbox`。
