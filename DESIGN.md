...
- One desk, one sheet: chrome sits on the desk; every plugin's work lies on the same rounded sheet.
- The tab strip always says where you are: a location chip for the plugin page, a cover chip for market and settings.

## Craft finish

Owned by `packages/design-system/src/styles/craft-finish.ts` (`CRAFT_FINISH_STYLES`, appended last to the workbench, settings and project-index stylesheets; `CRAFT_FINISH_CLIENT_SCRIPT`, part of `VISUAL_FOUNDATION_CLIENT_SCRIPT`). Decisions and verification: [specs/craft-finish](specs/craft-finish/spec.md). Catalog: `/__ui/catalog#craft`.

**Space.** The titlebar, plugin rail and directory are the desk (`--desk`, which is `--page`); nothing draws a line or shadow between them. Work is one sheet on the desk: `.tab-workspace` leaves 8px of desk on the right and bottom, and each pane and the exclusive cover is a 12px-rounded paper with `--sheet-shadow`. Split panes are separate sheets on the same desk; the focused one's edge reads a step darker. Pages inside the sheet do not draw a second card (Home's day view is flat on the sheet). Embedded panes and widths ≤ 600px keep no inset.

**Where am I.** When a pane shows a plugin's own page, the tab strip starts with a location chip — that plugin's glyph and name — marked current. Opening one of its items makes the chip a quiet "back to the plugin page" control. When market or settings covers the panes, a cover chip (插件市场 / 设置 / 项目设置) leads the strip and the covered plugin's chip stays clickable. Chips are not tabs: they never drag, group or share width.

**Rail.** Read top to bottom: this project, its tools, the ways to extend them, then you. The project header (monogram, name when names are shown, project settings gear, and the ⌘K search field) opens the rail. Tools follow in groups — Home with the work group in flow order (Goals, Inbox, Feed, Sessions, Schedule, 工作流程), then 创作 (Pages, Forms, Dataset, PPT, 图片, Artifacts), then 更多 (every other tool, including any Plugin added later, so adding a Plugin never edits the shell), then 拓展 (插件创作工作台, 插件市场, 能力). The person closes the rail: 灵光 · 对话 · 群聊, then settings and the account. In the icon rail groups are 16px hairlines and zones are 20px hairlines centred in the 8px between them (they take no height); with names shown the rail is 208px, groups become 11px headings, personal tools share one row and the account row carries the settings gear. The show-names toggle sits with Back and Forward at the left of the titlebar, where macOS keeps a sidebar button. On a phone personal tools live in the drawer, so the top of the page keeps one row less. Glyphs rest at 34% of their plugin tint mixed into `--muted`; hover and current use the full tint, and the current item adds a 3px tint mark at its leading edge beside the travelling chip. The project entry is the **project monogram**: its first letter on a hue chosen from the project id (renaming never recolours it), also used on project-index cards and in the project menu. Rail tooltips are one page-level element (never clipped by the scrolling column), appear only where the name is hidden, and name the shortcut: ⌘K for search, ⌥1…⌥9 for the first nine tools from Home. The shortcut is ignored while typing, in a terminal, or with a dialog or popover open.

**Depth.** `--lift-1` (resting card, selected directory row, current tab), `--lift-2` (hover, menus, canvas tools), `--lift-3` (dialogs), `--sheet-shadow` (the sheet). Directory rows selected on the desk become a small raised paper chip; rows selected inside the sheet keep the ~10% plugin wash. Corners: tag 5, row 8, control 9, card 12, sheet 12, dialog 16.

**Motion.** `--dur-press` 110ms, `--dur-hover` 140ms, `--dur-move` 220ms, `--dur-arrive` 320ms, `--dur-moment` 640ms on `--ease-quint`, `--ease-spring` and `--ease-swift`. A press gives: buttons, toggles, create controls and project cards scale to .97 and spring back; rail and titlebar glyphs dip to .84–.86. Controls that open a popover never scale, so their menu cannot shift. Hover is a tone step for rows and a one-level lift for cards (board cards and market cards also rise 1px). Arriving surfaces rise 6px while fading in; dialogs rise 10px from .975; edge sheets slide 24px from their edge; menus drop 4px from their trigger. The scrim only darkens — no blur. Toasts are a capsule with a success or failure mark; failure nudges once. Checkbox ticks draw themselves; a pressed switch knob stretches before it lands. Canvas nodes lift on hover and lift further while dragged; the selected Goal's dependency path flows as a dashed line. Under `prefers-reduced-motion`, and under automation (`navigator.webdriver`), nothing in this layer moves.

**Moments.** Only two things get one. When the open Goal's status turns from not-done to 已完成, its mark pops and a ring with a handful of sparks leaves it (opening an already completed Goal never does). When a new Goal, a newly pulled message or a new Inbox entry first appears, its row carries one plugin-tinted wash from its leading edge (at most three per update, so a redrawn list never flashes). Opening a project settles the rail in, then the sheet, once per page. Plugins may call `molisCraft.celebrate(element)` and `molisCraft.land(element)` for their own finish and arrival; both are presentation only.

**Home.** A greeting follows the local clock (早上好 · 中午好 · 下午好 · 晚上好 · 夜深了). The seven-day strip is its own half-desk column. When Home is at least 1080px wide, the first item of the day opens in an inspector column along the sheet's right edge with its primary action; closing it keeps it closed for this visit, and finishing one moves to the next. Narrower homes keep the single column.

**Density.** Feed's heading is one row (title and summary side by side) and a tab row with a single visible tab is hidden. Creating a Goal keeps its borderless writing lines; focus draws a 2px accent pen line under the active line. Fields answer focus with their 1px ink boundary plus a 3px ink halo at 7%; accent colour never draws a focus stroke.
...
An opened Goal fills its pane's sheet beneath the tabs: no further inset, square inner edges, no extra shadow. A compact toolbar with a 32px minimum height keeps Back on the left. Runtime owns the flexible main column; a closable 300px rail holds Goal information and the inline timeline. Below 840px Goal-workspace width, the rail overlays the work area at up to 330px. Goal reading and recording use the continuous paper surface, with fixed heading/actions and internally scrolling content; covered controls and return focus retain their existing behavior. Compact canvas nodes can move; opened workspaces cannot.
...
Motion expresses a state transition. Rows answer hover and press with a tone step; buttons, toggles and cards additionally give under a press and cards lift on hover (see Craft finish). Base timing: `--motion-instant` 90ms for hover and press, `--motion-fast` 130ms for boundary and elevation, `--motion-normal` 190ms for surface arrival, all on `--ease-standard` `cubic-bezier(.32, .72, 0, 1)` except arrival, which keeps `--ease-out` `cubic-bezier(.16, 1, .3, 1)`. Directory and plugin list rows use the 180ms arrival curve for hover fill so a long title can compress instead of flashing. Buttons answer a press with tone, and ghost controls with `--nav-press`; a control that opens a popover is excluded so its menu never shifts. Edge sheets slide in 24px from their edge. Centred modals rise 10px from .975 scale. Feed readers reveal over 160ms ease-out with a 3px upward starting offset; disclosure chevrons use 160ms ease. Project settings disclosures and switch knobs use 160ms ease. Reduced-motion preference suppresses animation, press feedback and smooth scrolling.
...
Icons are a curated Lucide library on a 24 grid, grouped as 栏 / 动作 / 对象 / 状态 in Catalog. Application surfaces render them at `stroke-width: 2`, which keeps a 16px glyph at a Linear-weight 1.33px stroke; large empty-state marks drop to 1.6 so they do not read as heavy. Chrome icon boxes are 16px. Plugin rail icons rest at 34% of that plugin’s `--plugin-tint` mixed into `--muted` and take the full tint on hover and when current; the current item adds the travelling wash and a 3px leading tint mark. Directory glyphs and empty-state marks inherit the same token when the surface has a plugin identity. Do not introduce a second icon family or emoji.
...
Home is a seven-day strip, the day's hero (greeting, date, summary) and the day's event stream, with an inspector column for the selected item; on wide homes the first item opens there on arrival (see Craft finish). There is no quotation carousel and no Home “打开 Goals” control; Goals is opened from the rail. No Agent Harness is implied.
...
- **Don't** give a plugin its own inset card inside the sheet, re-add fills to the rail groups, or blur the scrim.
