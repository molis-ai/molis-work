...
export const TAB_WORKSPACE_FACTORY_SCRIPT = `(host) => {
  ...
  const openTabMenu = (paneId, anchor, tabId = null) => {
    ...
    if (!tabId) {
      ["home", "goals", "sessions", "feed", "inbox", "schedule", "shelf", "lingguang", "functions", "form", "dataset", "ppt", "artifacts"].filter(plugin => plugin === "home" || document.querySelector('[data-plugin-strip] [data-plugin-id="' + plugin + '"]')).forEach(plugin => {
