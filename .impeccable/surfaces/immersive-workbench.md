...
Desktop default is Linear tool density, not a compact-mode opt-in. Directory chrome is one 36px row; plugin/Goal/Session rows are 28px; two-line directory objects are 36px; closed Feed rows are 40px source+title with the summary hidden until expand; tabs 32/26; shared controls 28px; reading titles 14–16px. Default sidebar is 240px, or 220px at ≤1050px. Narrow/coarse targets stay 44px. Home’s poetic composition, canvas node geometry, terminal type and domain contracts stay as they are. Density layer: `apps/workbench/src/styles/linear-density.ts`, last in the workbench/settings sheets. Contract: [Linear workbench density](../../specs/linear-workbench-density/spec.md).
...
- Top chrome: 32px work bar with 26px Chrome-style tabs. Project switching, search and project settings live in the titlebar (web: left of tabs; native desktop: right of tabs, after Overlay traffic-light inset). Notification placeholders and the top-right theme shortcut are absent; appearance settings retains theme control.
...
- Settings: global appearance, runtimes, planning methods and diagnostics open in the workbench directory from the plugin-rail gear; the account avatar no longer leaves the project. Project preferences remains a standalone Coss/Codex-style page with a 36px header, 224px category rail and content up to 920px. Headings are 16px / 550 / 1.3. The rail holds return, project identity and 常规 / 项目说明 / 工作规则 / 工作规划; workspace/plugin tabs and the workbench directory are absent. The content region owns its scrolling; project rules and guidance editing split fixed headings/actions from the scrolling fields. Guidance editing temporarily replaces its background list and cancel restores entry focus. Pending saves disable edits/cancel; failures preserve drafts with readable retry feedback. At 760px and below, return and four equal category links move above the content, headings remain 16px, controls use 44px minimum targets and form/storage rows stack. Independent `/settings/*` routes remain for the project index and direct links.
...
Home keeps local date/weekday, a small month calendar and the existing 13 quotations. It uses the shared neutral theme and serif quotation text. Date, calendar, quotations, shortcuts and the disabled composer share one 670px column, left-aligned in the pane and stacked from the top; leftover paper stays below. Container widths of 640px and 450px tighten and stack the composition. The composer remains disabled with “Agent 尚未开放”; no draft capture, message submission or simulated Agent response is introduced.

Quotations autoplay about every 8 seconds while Home is visible, with a 160ms opacity transition. Hovering or focusing the quotation pauses the cycle so source links stay clickable; there is no “换一句” control. All pages reserve the natural height of the longest text, with a 112px minimum; inactive pages retain layout but are hidden, inert and unavailable to accessibility. Reduced motion switches directly. Home has no “打开 Goals” control. Source attributions remain the originals; migrated English text is not rewritten by interface locale.
...
- [Project home](../../specs/poetic-project-home/spec.md), [quotation/footer follow-up](../../specs/home-narrow-footer-quotes/spec.md), [home shortcuts](../../specs/home-shortcuts-return/spec.md): retained home and shortcut behavior.
