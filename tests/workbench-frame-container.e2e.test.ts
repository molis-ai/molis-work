...
test("Goal Frames use outer tabs, preserve references, and isolate project state", { timeout: 90_000 }, async (t) => {
  ...
  await waitFor("document.querySelector('[data-titlebar-tabs] .tab-item[data-plugin=task][data-goal-id=CORE][aria-current]')");
  ...
  await evaluate("document.querySelector('[data-graph-node][data-goal-id=\"CORE\"] [data-graph-frame]').click()");
  await waitFor("document.querySelector('[data-titlebar-tabs] .tab-item[data-plugin=task][data-goal-id=CORE][aria-current]')");
  assert.equal(await evaluate("document.querySelectorAll('[data-titlebar-tabs] .tab-item[data-goal-id=CORE]').length"), 1);
  ...
  await evaluate("document.querySelector('[data-titlebar-tabs] .tab-item[data-goal-id=CORE]').click()");
  await waitFor("document.querySelector('[data-titlebar-tabs] .tab-item[data-plugin=task][data-goal-id=CORE][aria-current]') && !document.querySelector('[data-goal-frame-surface]').hidden");
  ...
  assert.equal(await evaluate("document.body.dataset.desktopSurface"), "task");
  ...
  await waitFor("document.querySelector('[data-goal-frame-surface]')?.dataset.frameGoal === 'CORE' && !document.querySelector('[data-goal-frame-surface]').hidden && document.querySelector('.tab-item[data-plugin=task][data-goal-id=CORE][aria-current]')");
  assert.equal(await evaluate("document.querySelector('[data-titlebar-tabs] .tab-item[data-goal-id=CORE]') != null"), true);
  ...
  await click('[data-plugin-strip] [data-plugin-id="goals"]');
  await waitFor("document.body.dataset.desktopSurface === 'goal' && document.querySelector('[data-titlebar-tabs] .tab-item[data-goal-id=CORE]')");
  await evaluate("document.querySelector('[data-titlebar-tabs] .tab-item[data-goal-id=CORE] [data-tab-close]').click()");
  await waitFor("!document.querySelector('[data-titlebar-tabs] .tab-item[data-goal-id=CORE]')");
  await evaluate("document.querySelector('[data-graph-node][data-goal-id=\"CORE\"] [data-graph-frame]').click()");
  ...
  assert.equal(await evaluate("document.querySelector('[data-titlebar-tabs] .tab-item[data-goal-id=CORE]')"), null);
  await navigate(() => command("Page.navigate", { url: origin + "/projects/" + projectId + "/" }, sessionId));
