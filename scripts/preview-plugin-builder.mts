/**
 * Local preview: real PluginPlatform/SQLite, intentionally no model credentials.
 * `--fixture` swaps in labelled fixed outputs for Prologue and Jev so the full build flow can be walked.
 */
...
import {PreviewFixtureRuntime,previewChoose} from './plugin-builder-preview-fixture.mjs';
...
const ports={store,boardId:DEMO_BOARD_ID,actorId:'preview-user',homeDirectory:directory,goalTitle:()=>undefined,escapeHtml,translate:(value:string)=>value,
 ...(process.argv.includes('--fixture')?{
  capabilities:new PreviewFixtureRuntime(directory),choice:{choose:previewChoose,selectionAvailable:()=>true},
  execution:{async ready(){},async models(){return [{provider_id:'fixture',model_id:'fixture-prologue',label:'预览替身 · 固定输出，非真实模型'}];}},
 }:{
  capabilities:{async invoke<Input,Output>(definition:{capability_id:string},_args:Input):Promise<Output>{
   if([agentHostCapabilities.listRuntimes.capability_id,projectSettingsCapabilities.workspaces.capability_id].includes(definition.capability_id))return [] as Output;
...
server.listen(portArg>0?Number(process.argv[portArg+1]):0,'127.0.0.1',()=>{const address=server.address();if(address&&typeof address==='object')console.log('Plugin Builder example preview: http://127.0.0.1:'+address.port+'/plugin-builder');});
