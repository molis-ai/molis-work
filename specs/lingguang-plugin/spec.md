...
4. 单条展开编辑、丢掉（确认）。丢掉把状态写成 `discarded`，不再出现在池里。
...
8. UI：`plugin-stage-shell` + `mw-*`。空态、列表、一条展开/编辑、底部快记、多选动作栏。确认用 `dialog.mw-dialog`。失败 `showNote`。
...
- 清空 = 清选择，不是清空整个池。
- 编辑在列表里展开；保存只补列表预览，不 `replaceChildren` 当前行。头脑风暴才切到 workspace。
...
2. 创建 → 列表倒序 → 重开 store 仍在；工作台 HTML 含 `data-lingguang=workbench`。
...
node --import tsx --test --test-concurrency=1 tests/lingguang-plugin.test.ts tests/creative-tools-plugins.test.ts tests/plugin-declarative-mounting.test.ts
node scripts/workspace-packages.mjs
```

浏览器主路径若本环境没有可操作的工作台，在交付里标未运行，不以 HTML 断言冒充手动能走通。
