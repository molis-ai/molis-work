...
test("Feed source navigation, independent nested panes, resize and restore", { timeout: 90_000 }, async (t) => {
  ...
  assert.equal(await evaluate("document.querySelector('[data-feed-stage-group]').dataset.feedStageGroup"), groupedSource);
  ...
  assert.equal(await evaluate("document.querySelector('[data-feed-task-title]').textContent"), "Solidot");
  ...
  assert.equal(await evaluate(`document.querySelector('[data-feed-item-wrap]:not([hidden])')?.closest('[data-feed-stage-group]')?.dataset.feedStageGroup`), source);
  assert.equal(await evaluate(`[...document.querySelectorAll('[data-feed-stage-group]')].filter(g=>!g.hidden).map(g=>g.dataset.feedStageGroup).join()`), source);
  ...
  await waitFor("document.querySelectorAll('iframe.tab-content-frame').length === 2 && [...document.querySelectorAll('iframe.tab-content-frame')].every(f=>f.contentDocument?.querySelector('[data-feed-task-title]')?.textContent==='Solidot')");
