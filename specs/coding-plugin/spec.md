...
- Prologue：`3077386c5b7bb86637dfd72b3405d3c629a5a066`，初始 `main` 干净。
  ...
  当前已改用 `prologue-sdk-0.0.0-rc.1-approved-receipts.tgz`：对应 Prologue 分支 `codex/molis-coding-receipts` 的提交 `46d8092e6057370012e8148c792d587f62a01275`，包含 MCP 资料范围/握手/分页、Node 交互输出、未声明工具的有界纠正与文件工具真实批准/落盘回执。SHA256 `852620648243866d755d0b733fbc8354e6cee2eb26b7c9bafa3f077025ead14a`；包内 496 个构建文件与 SDK dist 及 Molis 实际 node_modules 逐字节一致（c9-approved-receipts-sdk-consumer.json），声明、inventory 与锁文件配套，未发布 npm 包。
- 最新本机验证构建：4198，`2026-09-20T11:45:14.133Z`，源码摘要 `09efa6951d3695e70ea5002a48beaca74cbf3d402eb86f445ceb6055d86d00b2`；下文历史切片的构建和已知缺口按当时证据保留，当前状态以后续修复段落为准。
...
### 2026-09-20 接续盘点与本地提交

本次接续原 Coding Goal 后，用户要求先盘点并拆分本地提交。本轮不继续实现新功能、不推送或合并；下面的提交用于保存已有工作，不代表完整 Coding 交付。主工作树 `main` 与 FlyLeaf 未改动，继续保留上述两个隔离工作树。
...
| C9 上下文整理 | 插件 `agent.compaction` → Host 冻结 → Prologue compactor | 已接独立选择 Prompt v2、12,000 估计 token 触发、原文片段、取消及活动回放；MiniMax 实际整理后继续读文件，完整历史跨重启不变 | 已接未声明工具整批拒绝后一次有界纠正；新 MiniMax 完整任务保留案例数但仍有重复读取和错误解释，真实模型尚未再触发纠正分支，不能称长任务质量验收通过。整理请求用量尚未汇入主轮小计 |
| C9 检查点 / 恢复 | 右侧文件检查点 → Host Review → SDK | 已接本会话/授权根列表、完整预览、单次批准/拒绝、指纹冲突保护、Run/回退互斥和持久回执恢复。MiniMax 实际编辑后经正式 UI 回退；终态历史与草稿重启保留 | 未知副作用目前阻止重跑，显式核对与继续尚未接通；补真实批准回执后，MiniMax 指定任务已正确区分原编辑和手动回退；窄屏阅读已改善。不能将文件回退当命令/MCP 撤销或完整 C9 完成 |
