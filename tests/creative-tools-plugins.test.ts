...
test("Forms / Dataset / PPT 是个人插件，不进项目启用名单", () => {
  assert.deepEqual([...PERSONAL_PLUGIN_IDS], ["images", "experiments", "shelf", "lingguang", "functions", "characters", "pages", "form", "dataset", "ppt"]);
...
test("工作台客户端脚本在挂上三个创作插件后仍能解析", () => {
  ...
  assert.match(script, /\["images","experiments","shelf","lingguang","functions","characters","pages","form","dataset","ppt"\]/);
