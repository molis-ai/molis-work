...
允许：`specs/archive/function-scene-action-scope/`、`docs/platform/PLUGIN-DEVELOPMENT.md`、`docs/platform/PLUGIN-PLATFORM.md`、`docs/SSOT-MATRIX.md`、`packages/contracts`（functions 合同）、`apps/local-host` 行为总表、`plugins/native/feed` Manifest 与详情按钮、`plugins/native/functions` 去向文案、`modules/functions` README 一行、对应测试、与本标准冲突的旧 spec 句子。
