...
test("project workbench adds tabs without replacing existing ones and keeps split panes", { timeout: 90_000 }, async (t) => {
  ...
  await waitFor("document.body.dataset.desktopSurface === 'goal' && !document.querySelector('.tab-item[aria-current]') && !document.querySelector('.tab-item[data-tab-kind=mother]') && document.querySelector('[data-workspace-history=back]')?.disabled === false");
  ...
  assert.equal(await evaluate("document.querySelector('.tab-item[data-tab-kind=mother]')"), null);
  ...
  await click('[data-plugin-strip] [data-plugin-id="goals"]');
  await waitFor("document.body.dataset.desktopSurface === 'goal' && document.querySelector('[data-goal-momentum]')?.dataset.loaded === 'true' && !document.querySelector('.tab-item[data-tab-kind=mother]') && !document.querySelector('.tab-item[aria-current]')");
  assert.equal(await evaluate("document.querySelector('.tab-item[aria-current]')"), null);
  assert.equal(await evaluate("document.querySelector('.tab-item[aria-current]')?.hasAttribute('data-preview') || false"), false);
  ...
  assert.equal(await evaluate("document.querySelector('.immersive-titlebar [data-titlebar-tabs] .tab-item[data-tab-kind=home]') != null"), true);
  ...
  await waitFor("document.body.dataset.desktopSurface === 'goal' && !document.querySelector('.tab-item[aria-current]') && !document.querySelector('[data-goal-canvas-shell]')?.hidden && document.querySelector('[data-goal-canvas-shell]')?.dataset.boardView === 'list' && document.querySelector('.tree-node[data-select-goal=\"INTERFACES\"]')?.getClientRects().length > 0");
  ...
  await waitFor("document.body.dataset.desktopSurface === 'sessions' && !document.querySelector('.tab-item[data-plugin=sessions]') && !document.querySelector('.tab-item[aria-current]')");
  ...
  assert.equal(await evaluate("document.querySelector('.tab-item[data-plugin=sessions][data-tab-kind=mother]')"), null);
  assert.equal(await evaluate("document.querySelector('.tab-group[data-tab-group]')"), null);
  ...
  await waitFor("document.querySelectorAll('.tab-item').length === 0 && document.body.dataset.desktopSurface === 'goal'");
