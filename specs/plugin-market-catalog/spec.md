...
- 搜索是带图标的整行字段，只有 `mw-input-group` 一层描边，不在 input 上再画一圈；项目选择收到标题右侧，文案为「添加到」。可见控件是工作台 popover 菜单，不是系统 `<select>` 列表；隐藏的 `[data-market-project]` 仍是程序与 e2e 的真相源。框层见 `specs/plugin-market-search-frame/spec.md`。
