...
test("Sources mutation and Feed reload preserve utility state while fresh Goal links still open Goals", { timeout: 60_000 }, async t => {
  ...
  await click('[data-feed-add] [data-feed-add-toggle]');
  await waitFor("Boolean(document.querySelector('[data-feed-add].is-open'))");
  await click('[data-feed-add-form] [data-feed-sources-open]');
