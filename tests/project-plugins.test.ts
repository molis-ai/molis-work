...
test("Project plugin HTTP persists only the selected project, rejects unauthorized input, and repeats without duplicate facts", async t => {
  ...
  assert.deepEqual(catalog.listProjectPlugins(first.project_id), ["goals", "task"]);
  assert.equal((await add(second.project_id, "sessions", false)).status, 403);
  assert.deepEqual(catalog.listProjectPlugins(second.project_id), ["goals", "task"]);
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    assert.deepEqual(await response.json(), { project_id: second.project_id, plugins: ["goals", "sessions", "task"] });
  }
  assert.deepEqual(catalog.listProjectPlugins(first.project_id), ["goals", "task"]);
  const reopened = await openMolisWorkProjectCatalog({ homeDirectory });
  try { assert.deepEqual(reopened.listProjectPlugins(second.project_id), ["goals", "sessions", "task"]); }
  ...
  const index = await (await fetch(origin + "/api/settings/project-plugins")).json();
    assert.deepEqual(index.projects.find((item: { project_id: string }) => item.project_id === second.project_id).plugins, ["goals", "sessions", "task"]);
});
...
test("Adding Feed also enables Inbox, Inbox can be added alone, and repeats stay idempotent", async t => {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    assert.equal(response.status, 200);
    assert.deepEqual(await response.json(), { project_id: project.project_id, plugins: ["feed", "goals", "inbox", "task"] });
  }
  assert.deepEqual((await (await add(inboxOnly.project_id, "inbox")).json()).plugins, ["goals", "inbox", "task"]);
...
test("Catalog migration preserves all old project entries and keeps newly created project defaults after reopen", async t => {
  ...
  assert.deepEqual(migrated.listProjectPlugins(project.project_id), ["artifacts", "feed", "goals", "inbox", "sessions", "task"]);
  const created = await migrated.createProject({ display_name: "新项目", actor_id: "test" });
  assert.deepEqual(migrated.listProjectPlugins(created.project_id), ["goals", "task"]);
  migrated.close();
  const reopened = await openMolisWorkProjectCatalog({ homeDirectory });
  try {
    assert.deepEqual(reopened.listProjectPlugins(project.project_id), ["artifacts", "feed", "goals", "inbox", "sessions", "task"]);
    assert.deepEqual(reopened.listProjectPlugins(created.project_id), ["goals", "task"]);
