...
test("shared tokens describe Coss control radius, height, and translucent borders", () => {
  assert.match(VISUAL_FOUNDATION_STYLES, /--radius-control: 10px;/);
  assert.match(VISUAL_FOUNDATION_STYLES, /--radius-surface: 12px;/);
  assert.match(VISUAL_FOUNDATION_STYLES, /--control-h: 32px;/);
  assert.match(VISUAL_FOUNDATION_STYLES, /--control-border: color-mix\(in srgb, var\(--ink\) 8%, transparent\)/);
  assert.match(VISUAL_FOUNDATION_STYLES, /Coss control language: buttons, fields, dialogs/);
});
...
test("workbench, settings, and project-index keep the Coss control layer after later surface CSS", () => {
  ...
  const marker = "Coss control language: buttons, fields, dialogs";
...
test("primary actions stay Action fill with the control radius", () => {
  const workbench = renderMolisWorkWorkbenchStylesheet();
  const coss = workbench.slice(workbench.lastIndexOf("Coss control language: buttons, fields, dialogs"));
