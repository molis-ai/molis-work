...
## 2026-09-24 自开发修复：低高度桌面导航

本轮用户已授权按任务提交并推送功能分支；本节覆盖上方历史记录中的“不提交”，不授权合并、发布或改正式用户库。

实测 756×469 时，上方 Assistant 98px、项目卡片 132px、底部入口 178px，把中间列表压至 8px；正常 scrollIntoView 后 Shelf 中心被市场按钮覆盖。复用原结构，在 >600px 宽、≤600px 高且细指针的桌面窗口收紧左栏卡片和底部按钮至现有紧凑尺寸，保留全部入口、顺序、48px 栏宽和滚动。普通高度、触控 44px 与 ≤600px 抽屉保持既定行为。不得通过隐藏入口、二维挤排、强制点击或削弱断言通过检查。

验收：低高度中部能完整显示至少一个按钮，列表与 footer 不重叠；正常滚动后 Shelf 与列表末尾入口能被实际点击；普通桌面与手机抽屉回归。交互代码由正式 Coding Agent 提案与审查实施，外部仅复核和提交。未覆盖设备与主观手感仍待验收。

指定路径结果：Coding Agent 实际提出并实施样式和回归测试，工作台构建及跨插件 3 项检查通过；最终全仓构建、类型与包边界通过。更新构建重启后，真实页面 756×469 细指针视口的中间列表约 87px，底部在视口内；Shelf、末尾图片和底部 Characters 入口分别打开。390×844 抽屉的 Shelf 按钮仍约 44px 高且可点击。普通高度恢复正常布局。真实触控设备、极低高度、中文 IME 与主观手感未验收；不以这些结果覆盖所有窗口或整体体验。
...
| 标签条 / 标签 | 44 / 34 | 32 / 26；未钉住标签贴内容、超出均分，钉住仍是图标 |
| 共享控件 | 32px | 28px |
| Feed 关闭行 | 88px + 两行摘要 | 40px；来源+标题；摘要只在展开后出现 |
| Feed 工具条 | 52px | 32px |
| Frame / Inbox / Artifact / Session 标题 | 18–22px | 14–16px |
...
4. 项目/全局设置：设置文档退出 Linear 压缩，用 Codex 偏好节奏（见 `specs/settings-codex-surface/spec.md`）。
...
4. **用工作台末尾密度层覆盖共享控件。** Coss `--control-h` 在桌面 immersive 收到 28px；设置文档保持 32px；窄屏/coarse 仍按现有 44px 作用域。
