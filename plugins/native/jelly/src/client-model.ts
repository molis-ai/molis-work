...
export const JELLY_MODEL_CLIENT_SCRIPT = String.raw`
  const openModelSettings=async()=>{
    ...
    let providerId=settings.selection?.provider_id||settings.providers?.find((provider)=>provider.enabled!==false)?.provider_id||settings.providers?.[0]?.id||'';
    let modelId=settings.selection?.model_id||'';let custom=false;let apiFormat='openai-chat-completions';
    ...
    const paint=()=>{
      ...
      $('[data-jelly-model-status]').textContent=L(!providers.length&&!custom?'还没有可用模型。可以填写已有服务的地址和密钥。':settings.configured?'已有模型配置；可在这里切换 Jelly 使用的模型。':'配置后即可提炼摘要和拆解。按原文逐行拆分无需模型。');
    };
