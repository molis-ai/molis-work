...
旧确认不能覆盖后来的输出选择，已保存报告和历史来源不受切换影响。Coding「产物」面现在从原 Artifact 查询可读取的已保存报告，支持标题搜索、刷新和打开原固定报告；未保存预览不会混入。通用成果页展示保存时正文，并可返回原报告/会话。Shelf 实际消费、其他成果类型、Character 与协作链仍待接通。
