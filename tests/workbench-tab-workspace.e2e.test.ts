...
test("project workbench previews on click, commits on double-click, and keeps split panes", { timeout: 90_000 }, async (t) => {
  ...
  await click('[data-plugin-strip] [data-plugin-id="task"]');
  await waitFor("document.querySelector('.tab-item[data-plugin=task][data-tab-kind=mother][aria-current]') && document.querySelector('[data-task-detail-empty] [data-task-create]')");
  await click("[data-task-detail-empty] [data-task-create]");
  await waitFor("document.body.dataset.desktopSurface === 'task' && document.querySelector('.tab-item[data-plugin=task][data-tab-kind=item][aria-current]') && !document.querySelector('.tab-item[aria-current]').dataset.goalId");
  assert.equal(await evaluate("document.querySelector('[data-frame-goal-work]')?.hidden"), true);
  assert.equal(await evaluate("document.querySelector('[data-frame-goal-locate]')?.hidden"), true);
  await click('.tab-item[aria-current] [data-tab-close]');
  await waitFor("document.querySelector('.tab-item[data-tab-kind=home][aria-current]') || document.querySelector('.tab-item[data-plugin=task][data-tab-kind=mother][aria-current]')");
  ...
  const canvasTabId = await evaluate<string>("document.querySelector('.tab-item[aria-current]').dataset.tabId");
  await click('.tree-node[data-select-goal="CORE"]');
  await waitFor("document.querySelector('[data-goal-frame-surface]')?.dataset.frameGoal === 'CORE' && document.querySelector('.tab-item[aria-current][data-plugin=task]')");
  assert.equal(await evaluate("document.querySelector('.tab-item[aria-current]').dataset.tabId"), canvasTabId);
  ...
  await waitFor("document.querySelector('.tab-item[aria-current][data-plugin=task][data-goal-id=CORE]') && !document.querySelector('.tab-item[aria-current]').dataset.preview");
  ...
  await waitFor("!document.querySelector('.tab-item[data-tab-kind=item][data-goal-id=CORE]')");
  ...
  await click('[data-plugin-strip] [data-plugin-id="goals"]');
  await waitFor("document.querySelector('.tree-node[data-select-goal=CORE]')?.getClientRects().length > 0");
  await click('.tree-node[data-select-goal="CORE"]');
  await waitFor("document.querySelector('.tab-item[data-plugin=task][data-goal-id=CORE][aria-current]') && !document.querySelector('.tab-item[aria-current]').dataset.preview");
  await command("Emulation.setDeviceMetricsOverride", { width: 1440, height: 1000, deviceScaleFactor: 1, mobile: false }, sessionId);
