...
test("AI uses only selected snapshots, persists review before explicit idempotent save and keeps old citations", () => run(async store => {
  ...
  const draft = await generateCogniaDraft(run => run(store),{mode:"synthesize",material_ids:[source.id]}, {completeText:async prompt => { assert.ok(prompt.includes("不可信资料"));assert.ok(prompt.includes("Original evidence"));assert.ok(!prompt.includes("UNSELECTED CONTENT"));store.commit(store.preview({kind:"markdown",name:"vault",locator:"local:/vault",files:[{path:"source.md",data:Buffer.from("changed").toString("base64")}]}).id);return "# Knowledge\nA finding [S1]"; }});
...
test("natural Chinese questions retrieve relevant evidence and respect domain; empty evidence avoids model",()=>run(async store=>{
  ...
  const draft=await generateCogniaDraft(run => run(store),{mode:"query",question:"知识导入有什么原则？",domain_id:domain.id},{completeText:async prompt=>{calls++;assert.ok(!prompt.includes("OTHER_DOMAIN_SECRET"));return "# 原则\n保留原文与来源 [S1]";}});assert.equal(calls,1);assert.equal(draft.references.length,1);
  await assert.rejects(generateCogniaDraft(run => run(store),{mode:"query",question:"火星河流"},{completeText:async()=>{throw Error("must not call");}}),/没有找到相关资料/);
}));
test("missing provider, failure, invalid citations, cancellation and context limit never create knowledge",()=>run(async store=>{
  ...
  await assert.rejects(generateCogniaDraft(run => run(store),input,{}),/尚未配置/);
  for(const output of ['missing heading','# x\nuncited','# x\n[S1] and [S9]'])await assert.rejects(generateCogniaDraft(run => run(store),input,{completeText:async()=>output}));
  const abort=new AbortController();await assert.rejects(generateCogniaDraft(run => run(store),input,{signal:abort.signal,completeText:async()=>{abort.abort();return "# x\n[S1]";}}),/取消/);
  const large=store.createMaterial({title:"large",body:"a".repeat(100_001)});await assert.rejects(generateCogniaDraft(run => run(store),{...input,material_ids:[large.id]},{completeText:async()=>"unused"}),/100,000/);assert.equal(store.drafts().length,0);assert.equal(store.materials().length,2);
}));
