...
左边 Codex 目的地（项目首页 + 已启用插件，下面换列表）仍以 directory-plugin-switcher 为准。Goal 关系画布、原 `.goal-canvas-open` 工作框、各插件列表与领域事实不另做一套。
...
| 工作区标签 | 放在 32px titlebar，样式沿用几天前的 Goal `container-tab`：平面字、当前 2px 底标、关闭钮始终可见。不在主区再做一条圆角签。 |
...
1. 打开项目只见首页标签，不是画布。
2. 点插件出现该组：组名是色条加彩色字，Goals 母标签文案是「画布」不是第二个 Goals。工作区标签在 titlebar。画布/看板在母标签页右上角切换。标题栏不叠 Goal 画布/看板。点 item 在焦点栏打开对应标签；同一栏不重复。
3. 条上可同时停着首页、Goals 组、Sessions 组；切 Tab 不拆掉其他组。
4. 可拆栏、可留空栏、可关栏；同一 Goal 能同时在两栏打开。关到只剩一栏时铺满主区，不留下半屏空白。
5. 关母标签不拆 item；关光最后一张标签后首页自动出现。
6. 刷新后按项目恢复组 / item / 分栏 / 焦点。市场和设置独占后能回到原布局。
7. 390 不分栏；Light / Dark 组颜色可读。`prefers-reduced-motion` 下无位移花活。
