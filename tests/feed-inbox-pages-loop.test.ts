...
test("Pages retries saved input, rejects concurrent generation and preserves later user edits", async () => {
  ...
  try {
    ...
    await assert.rejects(generatePagesFromMaterials(store, record), /尚未配置/);
    assert.equal(store.generation("project-a", record.request_id)?.status, "failed");
    let calls = 0;
    let finish!: (value: string) => void;
    const first = generatePagesFromMaterials(store, record, async prompt => { calls++; assert.match(prompt, /SUMMARY_EXPORT/); return new Promise(resolve => { finish = resolve; }); });
    await assert.rejects(generatePagesFromMaterials(store, record, async () => "duplicated"), /仍在生成/);
    finish("# 观察\n\n有边界的内容。");
    const saved = await first;
    assert.match(JSON.stringify(saved.document.body), /原帖未核验/);
    store.update(saved.document.id, { title: "用户改过的标题" }, "project-a");
    const replay = await generatePagesFromMaterials(store, record, async () => { calls++; return "should not run"; });
    ...
    await assert.rejects(generatePagesFromMaterials(store, { ...record, request_hash: "changed" }, async () => "text"), /已改变/);
...
test("Inbox re-evaluation is project scoped, preserves attention state and supports three next actions", async () => {
  const home = mkdtempSync(join(tmpdir(), "molis-loop-next-"));
  const dbPath = join(home, "project.db"); seedDemoBoard(dbPath); const store = new LocalProjectDatabase(dbPath);
  let calls = 0;
  const judgments: JudgmentPort = {
    async judge(input) { calls++; assert.equal(input.scene_id, "inbox.next"); assert.ok(input.offered_behavior_ids.includes("inbox.verify"));
      return { judgment_id: "next-" + calls, function_key: input.function_key, function_version: 1, subject: input.subject,
        scene_id: input.scene_id ?? null, outcome: input.input.includes("uncertain") ? "needs_review" : "ok",
        suggested_behavior_ids: input.input.includes("uncertain") ? [] : ["inbox.compose"], error_code: null, created_at: new Date().toISOString() }; },
    bindScene(scene_id, function_key, board_id) { return { scene_id, function_key, board_id: board_id ?? null, ref: null }; }, unbindScene() {},
    sceneBinding(scene_id) { return scene_id === "inbox.next" ? { scene_id, function_key: "next", board_id: DEMO_BOARD_ID, ref: null } : null; }, latest() { return null; },
  };
  try {
    const feed = createLocalFeedApplication(store.db, { judgments });
    const source = createLocalFeedSourceService(store.db, DEMO_BOARD_ID).register({ kind: "research_library", repository: "molis-ai/research-library", research_source: "twitter-ai-observation" }).source;
    const entries = ["supported", "uncertain"].map(title => {
      const item = feed.ingestItem({ source, externalId: title, title, summary: title, occurredAt: new Date().toISOString(), attention: false }).item;
      return feed.ensureInboxEntryForFeedItem(DEMO_BOARD_ID, item.item_id, "manual").entry;
    });
    const result = await feed.evaluateInboxEntries(DEMO_BOARD_ID, entries.map(e => e.entry_id));
    assert.deepEqual(result.judgments.map(j => j.outcome), ["ok", "needs_review"]);
    assert.ok(entries.every(e => feed.getInboxEntry(DEMO_BOARD_ID, e.entry_id).status === "open"));
    await assert.rejects(feed.evaluateInboxEntries("other-project", [entries[0]!.entry_id]));
    await assert.rejects(feed.evaluateInboxEntries(DEMO_BOARD_ID, [entries[0]!.entry_id, "foreign-entry"]));
    assert.equal(calls, 2);
    feed.setInboxEntryStatus(DEMO_BOARD_ID, entries[0]!.entry_id, "dismissed", entries[0]!.revision);
    await assert.rejects(feed.evaluateInboxEntries(DEMO_BOARD_ID, [entries[0]!.entry_id]), /重新打开/);
    assert.equal(calls, 2);
...
test("Multi-material document retains every source and a project-scoped return link", async () => {
  ...
  try {
    ...
    const result = await generatePagesFromMaterials(store, multi, async prompt => {
