...
- 依赖：`specs/archive/functions-system-capability/spec.md` 的判断与现场绑定合同。

## 文件 / 模块边界

允许：`specs/archive/functions-product-authoring/`、`packages/contracts`（functions 合同）、`modules/functions`（列与内置 seed）、`plugins/native/functions`（UI/HTTP/文案）、`apps/local-host`（catalog 注入）、对应测试、SSOT 一行。
