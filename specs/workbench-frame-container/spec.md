...
状态：2026-09-14 锁定所有权。容器是壳；Frame 是按 Goal 绑定的工作面能力（不是左目录插件，也不塞进 Goals）；目录插件只提供可拖资产，拖入只存引用。切片外观不变：`docs/design/workbench-frame-container/`。`plan.md` 仍是旧模型，视觉锁定前不要执行。

本文件是本次行为变更的唯一需求书。它取代沉浸式工作台里「右侧主区由当前插件整页独占、Goal 点开即固定工作框」的主区模型；不取代左侧两层目录、项目首页、Goals / Work / Feed / Artifacts 的领域事实，也不恢复已废止的顶部 Goal tabs。

## 背景与目标

工作台现在是左目录 + 右插件整页。点 Goal、Session、Feed、交付物，主区就被该插件换掉；不能同时摊开几份工作，也不能按项目记住一套打开中的标签和窗口。旧个人工作台有过项目级 Goal 标签，沉浸式改版已明确废掉。

目标：工作台主表面是 **Goal 画布**。每个 Goal 可以在卡片上直接对话推进，或展开成该 Goal 自己的 **Frame**；Session / Feed / 交付物等只能拖进当前 Frame，不能进 Goal 画布。打开的 Tab 按项目暂存。当前完成等级 **2：可交互切片**；生产接线仍等视觉锁定。不宣称可发布；不改已安装 App、不写用户真实库做演示。
...
- 钉住的第一个 Tab 是 **Goal 画布**：无限画布，只放本项目的 **Goal 卡片**。
- Goal 卡片右上角两个模式：**直接对话推进**、**展开该 Goal 的 Frame**。
...
多 Window 拆分、撕出视口、Browser/Terminal 作为与 Goal 画布平级的顶层 Tab：基础概念可留，**不作为当前切片主角**。视觉锁定前不进入 `plan.md` 生产任务。
...
| Goal 画布 | 壳层 | 主画布；只放 Goal 卡片；不接收非 Goal 拖入 |
| Goal 卡片 | 壳层摆放 / Goals 拥有事实 | 主画布上的一张 Goal；右上角两个模式；对话槽归 Goals |
...
1. 打开项目 → 顶栏第一个 Tab 是 Goal 画布，上面是本项目 Goal 卡片。
2. 点目录「写周报」→ 回到 Goal 画布，选中该卡片。不再每次点击都新开 Frame。
3. 卡片右上角切到对话 → 在卡片里直接推进（生产接 Runtime；切片只示意）。
4. 卡片右上角展开 Frame → 顶栏出现「写周报」Frame Tab 并聚焦。再点展开只聚焦已有 Tab。
5. 在该 Frame 里从目录拖入 Session / Feed / 交付物 → 画布增加 Block。拖到 Goal 画布则拒绝。
6. 切到另一项目 → 换那套 Goal 画布 + 已展开 Frame。切回来原样。
7. 关某个 Goal 的 Frame Tab → 回到 Goal 画布；该 Goal 卡片还在。再展开可打开同一 Frame 构图。
...
3. **两种推进入口在卡片上。** 对话推进不离开卡片；展开 Frame 才进入该 Goal 的工作面。二者不是顶栏上的两个平级 App。
4. **非 Goal 只能进 Frame。** 目录拖拽的目标是「当前 Frame」。当前停在 Goal 画布时拒绝，并说明要先展开 Frame。
5. **点目录 Goal 是定位，不是开窗。** 选中主画布卡片。打开 Frame 只走卡片上的展开。
...
| 卡片「对话推进」 | 该卡片进入对话模式，仍停在 Goal 画布 |
| 卡片「展开 Frame」 | 打开或聚焦该 Goal 的 Frame Tab |
| 点目录非 Goal（当前是 Frame） | 可加到当前 Frame；切片里点击与拖入等价 |
| 点目录非 Goal（当前是 Goal 画布） | 拒绝，提示先展开 Frame |
...
| 数据 | 存什么 | 拥有者 | 性质 |
| --- | --- | --- | --- |
| Goal 画布 | 各 Goal 卡片位置、当前卡片/对话模式 | 壳摆放；Goal 事实与对话归 Goals | 项目工作内容 |
...
- **Goal 卡片表面**：Goals 提供卡片内容与对话槽；壳管摆放、选中、两种模式入口。
...
- 输入：当前 `project_id`、目录点击/拖拽、卡片模式、展开 Frame、画布指针手势、项目切换。
...
- `apps/workbench`：Container、Goal 画布、Tab 条、拖拽路由、项目切换恢复。不把 Frame 构图写进壳状态充当局域库。
- Frame 工作面：独立模块（内置 native plugin 或同等合同的 Host 模块均可），拥有按 `goal_id` 的构图读写与 Frame 画布。无 `workbench.directory` contribution。
- `packages/ui-host` 与 `packages/contracts`：卡片槽、Block 槽、可拖资产合同；不把领域事实塞进 Host。
- `apps/local-host`：项目范围的 Goal 画布暂存、Frame 构图、打开中的 Tab。
- `plugins/native/goals|work|feed|artifacts`：资产列表、Goal 卡片与对话、Block 表面。不在插件内做 Tab 条，Goals 不宿主 Frame 画布。
...
| 项 | 通过条件 |
| --- | --- |
| Goal 画布 | 顶栏有钉住的 Goal 画布 Tab；画面上只有 Goal 卡片 |
| 目录 Goal | 点 Goal 只选中主画布卡片，不新开 Tab |
| 对话 / 展开 | 卡片右上角两个模式可用；展开后出现该 Goal 的 Frame Tab，再展开聚焦同一 Tab |
...
浏览器走通场景 1–7。生产接线后再补 typecheck / 定向测试。

## 假设与开放问题

已锁定假设：Goal 画布是主画布；一 Goal 一 Frame；对话是卡片上的模式；点目录 Goal 只选中卡片；非 Goal 只能进 Frame；**容器是壳、Frame 是按 Goal 的工作面能力、目录插件只提供可拖资产**。

实现时再定、不阻塞当前切片：

- Frame 工作面第一期落在 `plugins/native/frame` 还是 Host 内同等合同模块（产品行为相同）。
- 对话是卡片内展开，还是卡片变对话面（切片按卡片内模式）。
...
- `specs/immersive-workbench-implementation/spec.md`：保留左目录、项目首页、插件按项目添加。主区从「插件整页 + Goal 固定框」改为 Goal 画布 + 按 Goal 的 Frame。
- `specs/goal-canvas-workspace/spec.md` 与 Goals 依赖图：仍是 Goals 插件内表面，不是工作台 Goal 画布，也不是 Frame。
