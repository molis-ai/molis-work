...
Native window drag regions and actual fullscreen-aware safe insets remain platform-owned. Tabs, buttons and resizers must remain outside native system controls and must not become drag regions. Overlay traffic lights sit in the 32px titlebar (`trafficLightPosition.y = 10`). The plugin rail starts on the next row. Project index remains a full-page arrival surface with its own brand, search and device-settings entry.
...
- Other existing menus/dialogs retain `--control-shadow`. The home composer has no shadow.
...
Destinations are plugin section headers: icon and label, with a disclosure control when the plugin has a list. Flat tone and stronger text mark the current plugin. Lists nest under their plugin and may stay expanded together. Goal and Session rows are 28px and single-line, with a title and compact status; Inbox and Artifact lists use compact two-line rows where applicable, with a title and one secondary fact. Feed source tasks show name, state and count in this directory. Empty states describe the missing content and offer only a real available action.
...
Home shortcuts remain project-local preferences on the project home page, above the disabled composer. Their modal supports add/edit/remove/cancel, keeps input on failure, restores focus, and opens validated http(s) links through the existing web/native path. They do not create Goal records.
