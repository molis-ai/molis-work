...
export function handleImagesRoute(service: ImagesService, input: {
  ...
  if (path.length === 1 && path[0] === "connections") {
    ...
    if (method === "POST") return { status: 200, body: { connection: service.saveConnection(body as unknown as ImageConnectionInput) } };
  }
  if (path[0] === "jobs") {
    ...
    if (path.length === 1 && method === "POST") return { status: 202, body: { job: service.start(projectId, body as unknown as ImageGenerateInput) } };
