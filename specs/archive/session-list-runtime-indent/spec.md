...
本文件补 `specs/archive/session-stage-list/spec.md`：Runtime 分类是组，Session 是组里的内容，条目不能和「Claude Code / Codex」标题齐平。缩进档位对齐 `specs/archive/goal-list-collection-indent/spec.md`（16px）。
...
不做：不改 Goals 缩进合同、Session 与 Goal 关系、用户库。Feed / Inbox 见 `specs/archive/plugin-stage-collection-indent/spec.md`。
