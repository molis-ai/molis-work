import { ActionError, type ActionCallContext, type ActionDefinition, type ActionHandlerBinding, type ActionReference, type ActionSchema } from "@molis-ai/molis-work-contracts/platform/actions";
...
export interface InboxPagesInput { request_id: string; entry_ids: string[]; title: string; instructions: string }
export const inboxActions = {
  ...
  readJudgment: action<Record<string, never>, InboxJudgmentState>("inbox.judgment.read", "下一步判断规则", "读取绑定的判断规则及适用的已发布规则", "query", empty, {
    type: "object", properties: { function_key: functionKey, functions: { type: "array", items: { type: "object",
...
export interface InboxActionPorts {
  ...
  generatePages?(input: InboxPagesInput, caller: ActionCallContext): Promise<InboxGeneratedPages>;
...
export function createInboxActionHandlers(ports: InboxActionPorts): ActionHandlerBinding[] {
  ...
  return [
    ...
    bind(inboxActions.evaluateJudgment, (input, caller) => ports.evaluateJudgment!(input.entry_ids, caller), !!ports.evaluateJudgment),
  ];
}
