...
test("Immersive directories resize and retain compact, operable Goal, Feed and Session lists", { timeout: 90_000 }, async t => {
  ...
  await waitFor("document.querySelector('[data-plugin-section=shelf]')?.hidden === false && !document.querySelector('[data-workspace]').classList.contains('is-plugin-directory-empty') && document.querySelector('#goal-tree-pane').getBoundingClientRect().width > 44");
  ...
  await click('[data-plugin-strip] [data-plugin-id="shelf"]');
  await waitFor("document.querySelector('[data-plugin-section=shelf]')?.hidden === false && !document.querySelector('[data-workspace]').classList.contains('is-plugin-directory-empty')");
