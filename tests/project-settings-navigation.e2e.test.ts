...
test("project general settings persist a rename, cancel safely, and retry deletion after network failure and a lost response", { timeout: 60_000 }, async t => {
  ...
  await navigate(() => command("Page.navigate", { url: origin + prefix + "/settings/guidance?desktop=1" }, sessionId));
  await navigate(() => click('.project-settings-navigation a[href*="/settings/general"]'));
  assert.equal(await evaluate("document.querySelector('.project-settings-navigation .settings-nav-body a[aria-current=page]').textContent.includes('基本信息')"), true);
  ...
  assert.equal(await evaluate("document.querySelector('.settings-heading p').textContent"), "项目的新名称");
