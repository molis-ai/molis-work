...
Prologue Node 接上宿主审查队列时声明子任务能力 `partial`；当前只支持同一已授权根目录的只读子角色，CLI 和独立写入目录仍不可用。Host 从插件声明冻结子角色正文、版本和工具子集，SDK 在第一次子模型请求前再次核对本轮角色归属。派出、补充要求走原 Review；每个子任务自己的调用仍走 SDK 原链，不复制执行器。
