...
**Casebook 是 Molis Work 的 RSI（Recursive Self-Improvement，递归自我改进）插件，把真实使用中的困难转成可执行改进，验证实际效果，再改进自身的观察、判断和方案能力。** 它既接收系统发现的疑点，也接收成员主动反馈；两条入口汇合到同一份改进 Backlog。Casebook 判断问题分类与模块归属，团队全员能看所有模块的 Backlog；对应模块主 R 决定值得优化后，系统自动准备优化方案和必要的选型比较，主 R 确认或调整后进入正式 Goal、修复与回验。

两条闭环分别看：**Molis Work 的使用结果是否改善，以及 Casebook 是否更会发现和帮助解决问题。** 第二条利用误报、漏报、误分、卡片反馈和无效方案提出校准建议，复用同一 Backlog/Goal 与正式发布流程，不新增另一套任务或审批系统。RSI 不意味着自动提升权限、全量采集、自动训练或无人批准地改代码和发布；仓库/插件名称不因此迁移。
...
实线表示单个问题的推进；虚线表示效果与校准反馈，不是每条问题必经的审批。“全员”指同一内部团队，未提交的个人疑点不进入共享池。
...
flowchart TD
    ...
    S9["9 · 执行者<br/>认领并实施"]
    S10{"10 · 回验者<br/>比较实际改善"}
    S11["11 · 系统与维护者<br/>提出并验证 RSI 校准"]
...
为保持主线可读，未继续推进、等待与失败的处理写在对应编号的说明中：步骤 3 未提交不共享，步骤 4 未明确归属则保留待处理，步骤 5 需补充/不优化不进入方案准备，步骤 10 有改前/改后依据才记录范围内的改善结论。步骤 11 是旁路校准，不阻塞任何单条改进；返回步骤 4 的是有必要评审的自身改进建议，不是自动创建任务。旧 Goal 已收尾时，步骤 10 的后续处理遵守正式变更路径，不自动重开。
...
| 6 | 准备方案与必要选型 | 避免“决定要改”之后，仍让人从空白写方案 | 系统自动给出建议改法、依据、预期改善、改前基线、工作量/依赖、风险和回验方式；有实质取舍才比较备选，不编工期或基线。依据不足先给最小排查建议；失败可在原事项重试，原决定保留，不增加成员必填表。 |
...
| 10 | 比较实际改善 | 区分“代码写了”和“原来的困难真的改善了” | 核对实际运行版本，按方案中的基线重走原场景，检查正常流程、跨模块交接与权限是否退化；记录改善/未改善/变差/未知及证据与范围，回到原问题和后续使用。主观体验须真人判断，只完成排查不能算修好；已结束 Goal 后走正式继续/变更，不自动重开。 |
| 11 | 提出并验证 RSI 校准 | 让识别、分派、表达与方案能力也能从实际差错中改进 | 系统整理获权样本与反馈，维护者独立判读；必要的自身改进复用步骤 4–10。保留旧/新版本和独立评估，依有权决定与正式发布流程生效或回退。此步旁路进行，不要求普通成员标注；不把未提交/不优化当负例，不拿自身输出当新事实，也不自动训练/发布。没有可信差错不造任务。 |
...
RSI 增量仍落在以上分工：Casebook 负责效果比较、差错整理和校准建议；宿主通过已有正式来源提供可读的执行/运行版本、结果与权限边界。优先复用 D3/D4 和 H2–H4；有具体缺口才补窄能力，不要求登录团队开发分析器、另建“RSI API”或新发布服务。部署事实由实际发布/运行 owner 提供，Goal 关闭不能替代。
...
### H4 增量实现（2026-09-20，开发分支）

`codex/casebook-plugin-storage` 为公开 `PluginPrivateStorage` 增加可选 `compareAndSet`，SQLite Host 使用单条条件写入，并保留安装隔离与每次调用的 grant 检查。原有 get/set/delete 兼容不变，没有新增表或改动业务库结构。Casebook 可将一次事务的状态编码为一个带修订标记的值，通过该能力拒绝并发覆盖；不需要直接接触宿主数据库。

两个独立连接的旧快照覆盖、权限撤回、安装隔离测试，以及既有插件生命周期回归通过。此进展仅覆盖本地持久化/冲突处理的工程能力；未合入或部署，不代表真实账号、团队共享、事实采集或完整插件页面已接通。正式身份仍须由宿主传入，不能把固定 `web-user` 当成 Auth 用户。
...
上述验证检查宿主能力与协作链路；识别效果、归属与方案质量、卡片是否好懂由 Casebook 另行验证。步骤 10 需真实改善比较，步骤 11 需范围明确的差错定位、版本比较和独立评估；样本不足只记探索，不能说 RSI 已变好。不用 Mock、历史样本数量、采用率或接口返回成功冒充效果验收。完整标准由 Casebook 验收清单第 6 节维护；本次文档校准不改任何产品通过状态。
