...
## 2026-09-24 被动页面不启动 Agent（实现与定向验证完成）

实际调用栈确认：Coding 材料路径调用 Shelf snapshot，目录探测对本机 Claude 执行 `--help`，引发钥匙串读取。被动快照、设置与 Agent 目录改为只检查文件；执行能力保持未检查，页面显示“执行时检查”。动作入口仍可进入原确认界面，权限与隔离明确未确认；用户真正执行 Agent 动作时才检查所选程序，并按原规则拒绝不支持无界面任务的程序。不得遍历启动其他程序，本机文字提取不探测 Agent。显式 pathEnvironment 为完整搜索范围，不再追加系统目录；未指定时保留默认本机发现。

复用已有 ShelfRuntimeProbe 与动作前校验，不新增探测服务或后台轮询。合同新增可选 capability_pending，旧 can_run_job 仍只表示确认过的任务入口；待检查不能标成可运行或已登录。回滚代码即可，无数据迁移、凭据改动或安装替换。测试用本地可执行夹具记录启动，覆盖快照零启动、实际任务仍执行、失败保留；真实登录和安装包验收另行确认。

验证：Shelf 23 项定向通过；CLI 与 Coding 正式 HTTP 等 56 项定向通过，构建、边界和类型通过。被动页面相关路径与全量回归没有新增 `security` 调用。真实设置页面显示“执行时检查”，没有把可执行文件误报成已登录。全量不是全绿，完整基线对照与 尚未解决的 Goal 菜单测试时序问题记录在 `specs/coding-plugin/spec.md` 的本轮验证记录。正式安装未替换。
...
工作台信息架构以 `specs/archive/plugin-stage-master-detail/spec.md` 为准：Shelf 不占第二栏，材料 / 生成结果 / 剪贴板三个 fold 在舞台列表里；默认全宽，点一行才展开预览。页脚「副本工作区 / ⌘V 粘贴当前」钉在目录栏底，展开后仍在 213pt 左栏底。本文管 DropAgent 功能、预览质感和 `--da-*`。舞台列表底色跟其它插件用 `--paper`（见 `specs/archive/shelf-list-paper/spec.md`）；文件名跟其它插件目录行用 `--ink`、13px（见 `specs/archive/shelf-row-ink/spec.md`）；类型图标、时间戳、预览、底栏、轮盘仍走 DropAgent。
...
- 工作台里 Shelf 列表走 Goals 舞台主从（见 `specs/archive/plugin-stage-master-detail/spec.md`），不再占第二栏。预览 + 对照 + 底栏指令 + 对话仍是 DropAgent 工作面。
