...
允许改：`apps/workbench/src/tab-reorder.ts`、`scripts/client/tab-workspace.ts`、`styles/tab-workspace.ts`、`styles/linear-density.ts`（空位高度跟上标签）、`specs/archive/tab-split-drop-preview/spec.md` 里「栏中间铺满」的句子、`DESIGN.md` / surface 一句、相关测试、本 spec。
