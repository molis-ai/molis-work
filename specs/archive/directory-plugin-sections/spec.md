...
- 点插件名：在焦点栏添加该插件母标签（`specs/archive/additive-tabs/spec.md`）；若该段收着则一并展开。
- 点段的展开控件：只开/收列表，不换右边。
...
- 插件操作（筛选、新建、归档等）在该段标题下方单独一行，展开时可见；收起时只留图标和名称。当前壳层以 `specs/archive/chrome-plugin-rail/spec.md` 为准。
- 项目头和账号底栏不滚。
- 窄屏 600px 抽屉同一套分段。
...
- 不给目录画自定义滚动条或底部渐隐提示。
- 不把账号收进某个插件。
- 不把快捷方式挂在目录里；快捷方式位置以 `specs/archive/home-shortcuts-return/spec.md` 为准。
- 不恢复目录内搜索框；全局搜索仍在项目头。
...
- Goal 树「折叠全部」已由 `specs/archive/goals-directory-remove-collapse-all/spec.md` 删除；行级折叠仍只折叠 Goal 节点，不收起 Goals 这一段。
...
试用目录 `/var/folders/.../molis-dir-sections-*`，未打默认 home。快捷方式位置仍以 `specs/archive/home-shortcuts-return/spec.md` 为准。标签单击添加、不替代以 `specs/archive/additive-tabs/spec.md` 为准。
