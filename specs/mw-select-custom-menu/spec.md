...
- 新建 Session 的 Goal/Runtime 已用隐藏 select + 纸面 listbox（`specs/session-add-custom-select/spec.md`），但明确不覆盖其它页面。
...
允许：`specs/mw-select-custom-menu/`、`packages/design-system`（原语 CSS + 客户端脚本）、Dataset/Form/Pages 里针对 `.mw-select` 的宽度选择器、对应测试、胶囊页接入同一脚本。
