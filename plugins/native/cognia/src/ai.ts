...
export interface CogniaAiPorts { runtimeLabel?: string; completeText?: (prompt: string, options?: { signal?: AbortSignal }) => Promise<string>; signal?: AbortSignal }
export async function generateCogniaDraft(store: CogniaStore, input: Record<string, unknown>, ports: CogniaAiPorts): Promise<Draft> {
  requireCognia(ports.completeText, "尚未配置文字模型。请在宿主配置文字模型后重试；导入、搜索和阅读仍可使用。", 503);
  ...
  return store.addDraft({ id: randomUUID(), title, body, references, mode: input.mode, domain_id, saved_id: null, created_at: new Date().toISOString() });
}
