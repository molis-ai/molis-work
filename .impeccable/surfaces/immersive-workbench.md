...
- Shared foundation: 8px control corners, square continuous work/editor surfaces, 12px canvas nodes, 32px shared controls; near-black primary in Light and near-white in Dark. Active directory rows stay flat; active work tabs join paper without shadow.
- Shell: one directory plus flexible stage. Directory defaults to 280px, or 256px at up to 1050px; existing saved widths remain valid. Pointer/keyboard resize and double-click reset remain available.
- Top chrome: 44px work bar with 36px Chrome-style tabs, 172px wide or 144px at up to 760px, with 10px rounded upper corners. The directory top row contains search and collapse/reveal; the 52px project row gives the title room and keeps project settings. Notification placeholders and the top-right theme shortcut are absent; appearance settings retains theme control.
- Low-height adaptation: Session dialog headers and footers stay visible while fields scroll; mobile form height follows the dialog content box. The directory middle scrolls when needed, preserving project/account chrome and a minimum 180px active directory; normal-height lists keep their own scroll areas. Verified at 1024×400 and 390×500 in `low-viewport-v7` evidence.
- Directory: persistent project destinations, active plugin title/actions and list, shortcuts, then account/global settings. Goal rows are 36px and use compact text status without nested tag borders. Other object lists use title plus one secondary fact. No separator is inserted between destinations/list or above the footer. Account name/space remain two lines at 36px desktop and 44px in the drawer.
- Tabs: stable plugin-colored dot/name/disclosure and bottom rule, plugin mother page and item tabs; paper current tab; separate title trigger and stable close slot. Groups, tabs, focus and nested pane layout persist per project. Arrow/Home/End select; Delete closes; trigger focus survives redraw. Goal tab selection/restoration loads the corresponding body. Current tabs remain visible when viewport width changes.
- Goal: flush to the full pane under the tabs, with zero inset/radius/shadow/backdrop and a left Back action in a 48px-minimum toolbar. Flexible full-height Runtime sits beside a closable 300px information/timeline rail. Below 840px Goal-workspace width the rail overlays at up to 330px; mobile Goal work remains flush and square. There is no adjustable 70/30 Goal split. Terminal-only availability omits the duplicate mode bar.
- Canvas/board: real active relationships and retained completed nodes. Canvas click selects lineage; open control/double-click/Enter enters work. Board counts derive from actual column items, with an explicit empty-column state. History remains inline, with a 16px expanded-event heading.
- Settings: project preferences is a standalone Coss/Codex-style page with a 44px header, 224px category rail and content up to 920px. Headings are 20px / 600 / 1.4. The rail holds return, project identity and 常规 / 项目说明 / 工作规则 / 工作规划; workspace/plugin tabs and the workbench directory are absent. The content region owns its scrolling; project rules and guidance editing split fixed headings/actions from the scrolling fields. Guidance editing temporarily replaces its background list and cancel restores entry focus. Pending saves disable edits/cancel; failures preserve drafts with readable retry feedback. At 760px and below, return and four equal category links move above the content, headings remain 20px, controls use 44px minimum targets and form/storage rows stack. Global settings shares the independent preferences shell; project index retains its arrival scope.
- Feed: each source is a left-directory task with name, state and count. Selection synchronizes the source, stage title and filtered message list. Rows show provider/source/time/read state, a 14px title, 12px summary and disclosure. Expansion keeps one title, hides the repeated summary and reads inline at 14px/1.8 within 76ch; source links remain available. Optional materials follow the body, and destination/actions share a divided footer. The selected-source stage heading has no Feed eyebrow. At 760px and below, reader actions use two equal columns with 44px minimum heights. Search, filters and source management remain reachable. Inbox remains a separate attention surface.
- Phone: at up to 600px the directory becomes a 264px focus-managed drawer with inert background and scrim. Selecting an object closes it. Work tabs remain present; Goal details are available on demand. At narrow widths or with coarse pointer, tabs, group labels, Close/Split and the new Home/Frame-picker controls use 44px interactive targets. Fine-pointer desktop controls retain their compact scale.

## Nested work panes

The layout is a binary tree of horizontal and vertical splits, including T-shaped arrangements. Every pane has independently usable content, including two panes of the same plugin. Dragging reorders or moves tabs across panes; Alt/Option copies. Dropping on one of four pane edges splits with the actual dragged tab. The split control copies to the right, or below with Shift. A 5px separator supports pointer and keyboard resizing, with double-click to equalize. Closing a pane closes its tabs and expands the remaining layout. To retain a tab, move it to another pane before closing the source pane.
...
The focused pane synchronizes the current Goal title and directory selection without touching terminal bindings. A collapsed tab group retains the current tab and its keyboard path. Same-pane focus does not move or reload the work document. Splits preserve the current Frame/work view; an explicit “打开 Frame” selects Frame in the existing Goal tab even if that tab previously showed work. Loaded documents and unfinished forms retain their existing live continuity; full-refresh recovery of unsubmitted forms is not claimed.
...
The server renders the selected category’s actual content on the first GET. Settings no longer rewrites to the workbench or waits for a client-mounted stage. Direct URLs, refresh and browser history use the same independent page. 返回工作台 restores saved project tabs and splits; restoration clears obsolete `project-settings` exclusive state. Existing `embed=1` requests still return a fragment for project management. Project planning detail/edit paths share the preferences shell.
...
- Feed owns full source-message facts and source tasks; Inbox owns attention references with entry reason, related object and next step. Feed task configuration uses the attached editor; migration/import retains a compact prompt. Relay migration stays user-triggered and read-only against Relay, with secrets re-sealed into Molis-owned stores. External source content stays untrusted. Promote/Start creates or reuses a Draft and binds input; choosing a terminal can fill context but never auto-send.
...
Home keeps local date/weekday, a small month calendar and the existing 13 quotations. It uses the shared neutral theme and serif quotation text. The centered content remains at most 800px, upper context 670px, launch region 740px; container widths of 640px and 450px tighten and stack the composition. The composer remains disabled with “Agent 尚未开放”; no draft capture, message submission or simulated Agent response is introduced.

Quotations change only through the “换一句” action, with a 160ms opacity transition and no automatic rotation. The action uses the shared quiet Coss treatment, an aligned 14px icon and visible hover/focus states. All pages reserve the natural height of the longest text, with a 112px minimum; inactive pages retain layout but are hidden, inert and unavailable to accessibility. Reduced motion switches directly. The real “打开 Goals” action leads to Goals; both new Home actions have 44px targets at narrow widths or with coarse pointer. Source attributions remain the originals; migrated English text is not rewritten by interface locale.

Shortcuts live in the directory below the active list, not on the home canvas. Add/edit/remove/cancel remains a project-local device preference. Validation and save/open failures preserve an honest correction or retry path, closing restores focus, and http(s) destinations use the existing web/native external-open channel. Names render as text and icons are local Lucide symbols. These preferences do not write Goal truth or synchronize across devices.
...
| Global settings and arrival | `apps/workbench/src/settings-renderer.ts`, `project-settings-pages.ts`, `styles/project-settings-page.ts`, `styles/settings.ts`: independent preferences and planning library; `project-index.ts` retains arrival scope |
...
- [Initial immersive implementation](../../specs/immersive-workbench-implementation/spec.md) and [progress](../../specs/immersive-workbench-implementation/progress.md): original browser/data/PTY/plugin integration evidence.
- [Project home](../../specs/poetic-project-home/spec.md), [quotation/footer follow-up](../../specs/home-narrow-footer-quotes/spec.md), [directory shortcuts](../../specs/directory-shortcuts/spec.md): retained home and shortcut behavior.
- [Tab workspace](../../specs/workbench-tab-workspace/spec.md), [global search](../../specs/global-plugin-search/spec.md), [Feed stage directory](../../specs/feed-stage-directory/spec.md), [plugin catalog](../../specs/plugin-market-catalog/spec.md): existing navigation capabilities.
