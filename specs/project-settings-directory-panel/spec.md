...
本文件是这次入口变更的唯一需求书。它覆盖 `specs/settings-directory-panel/spec.md` 里「项目设置仍走独立页」，以及 chrome 上那颗项目齿轮当前整页跳到 `/projects/{id}/settings*` 的行为。全局设置按同一壳模型，见 `specs/settings-directory-panel/spec.md`。四个分类的字段、保存、审计仍以 `specs/project-settings-redesign/spec.md` 为准。
...
- 全局设置按同一模型改，见 `specs/settings-directory-panel/spec.md`。
